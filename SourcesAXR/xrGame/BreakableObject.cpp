...
void CBreakableObject::shedule_Update	(u32 dt)
{
	...
	if(m_pPhysicsShell&&!bRemoved&& CRenderDevice::GetInstance()->dwTimeGlobal-m_break_time>m_remove_time) SendDestroy();
}
...
void CBreakableObject::Break()
{
	...
	m_break_time= CRenderDevice::GetInstance()->dwTimeGlobal;
	SheduleRegister	();
}
