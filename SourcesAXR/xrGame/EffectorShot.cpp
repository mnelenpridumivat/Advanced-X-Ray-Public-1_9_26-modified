...
void CWeaponShotEffector::Shot( CWeapon* weapon )
{
	...
	angle      += m_cam_recoil.DispersionInc * weapon->cur_silencer_koef.cam_disper_inc * static_cast<float>(m_shot_numer);
	Shot2( angle );
}
...
//	Msg( " <<[%d]  v=%.4f  dv=%.4f   a=%d s=%d  fr=%d", m_shot_numer, m_angle_vert, m_delta_vert, m_actived, m_first_shot, CRenderDevice::GetInstance()->dwFrame );
}
...
void CWeaponShotEffector::SetRndSeed	(s32 Seed)
{
	if (m_LastSeed == 0)
	{
		m_LastSeed			= Seed;
//		m_Random.seed		(Seed);
		m_Random.seed		(CRenderDevice::GetInstance()->dwFrame);
	}
}
...
//		m_actived = false;
//	}
//	Msg( "[%d]  pitch = %.4f   yaw = %.4f    fs=%d    a=%d  fr=%d", m_shot_numer, *pitch, *yaw, m_first_shot, m_actived, CRenderDevice::GetInstance()->dwFrame );
