...
//	teams[ps->team-1].score++;
	...
	NET_Packet			P;
	P.w_begin			(M_EVENT);
	P.w_u32				(CRenderDevice::GetInstance()->dwTimeGlobal);
	P.w_u16				(GE_DESTROY);
...
void game_sv_ArtefactHunt::Update()
{
	...
	case GAME_PHASE_TEAM2_ELIMINATED :
		{
			if ( m_delayedTeamEliminated && m_TeamEliminatedDelay < CRenderDevice::GetInstance()->TimerAsync() )
			{
...
void game_sv_ArtefactHunt::Artefact_PrepareForSpawn()
{
	...
	m_dwArtefactSpawnTime = CRenderDevice::GetInstance()->dwTimeGlobal + Get_ArtefactsRespawnDelta()*1000;
...
{
	m_dwArtefactRemoveTime = CRenderDevice::GetInstance()->dwTimeGlobal + Get_ArtefactsStayTime()*60000;
...
bool				game_sv_ArtefactHunt::Artefact_NeedToSpawn	()
{
	...
	if (m_dwArtefactSpawnTime < CRenderDevice::GetInstance()->dwTimeGlobal)
	{
...
bool game_sv_ArtefactHunt::Artefact_NeedToRemove()
{
	...
	if (m_dwArtefactRemoveTime < CRenderDevice::GetInstance()->dwTimeGlobal)
	{
