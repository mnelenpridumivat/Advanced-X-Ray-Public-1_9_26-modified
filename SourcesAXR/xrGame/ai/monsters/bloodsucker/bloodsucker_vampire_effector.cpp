...
BOOL CVampireCameraEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	if (time_left_perc < 0.2f) {
		...
		angle_lerp(dangle_current.x, dangle_target.x, _abs(dangle_current.x / fLifeTime + 0.001f), CRenderDevice::GetInstance()->fTimeDelta);
		angle_lerp(dangle_current.y, dangle_target.y, _abs(dangle_current.y / fLifeTime + 0.001f), CRenderDevice::GetInstance()->fTimeDelta);
		angle_lerp(dangle_current.z, dangle_target.z, _abs(dangle_current.z / fLifeTime + 0.001f), CRenderDevice::GetInstance()->fTimeDelta);

	} else {
		
		if (angle_lerp(dangle_current.x, dangle_target.x, ANGLE_SPEED, CRenderDevice::GetInstance()->fTimeDelta)) {
			dangle_target.x = Random.randFs(DELTA_ANGLE_X);
		}

		if (angle_lerp(dangle_current.y, dangle_target.y, ANGLE_SPEED, CRenderDevice::GetInstance()->fTimeDelta)) {
			dangle_target.y = Random.randFs(DELTA_ANGLE_Y);
		}

		if (angle_lerp(dangle_current.z, dangle_target.z, ANGLE_SPEED, CRenderDevice::GetInstance()->fTimeDelta)) {
