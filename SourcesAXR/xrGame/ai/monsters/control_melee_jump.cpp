...
bool CControlMeleeJump::check_start_conditions()
{
	...
	if (m_time_next_melee_jump > CRenderDevice::GetInstance()->dwTimeGlobal)	return false;
...
void CControlMeleeJump::on_release()
{
	...
	m_time_next_melee_jump = CRenderDevice::GetInstance()->dwTimeGlobal + Random.randI(ROTATION_JUMP_DELAY_MIN,ROTATION_JUMP_DELAY_MAX);
}
