...
void CPhantom::UpdatePosition(const Fvector& tgt_pos) 
{
	...
	angle_lerp		(vHP.x,tgt_h,fASpeed, CRenderDevice::GetInstance()->fTimeDelta);
	angle_lerp		(vHP.y,tgt_p,fASpeed, CRenderDevice::GetInstance()->fTimeDelta);

	cur_dir.setHP	(vHP.x,vHP.y);

	Fvector prev_pos=Position();
	XFORM().rotateY (-vHP.x);
	Position().mad	(prev_pos,cur_dir,fSpeed* CRenderDevice::GetInstance()->fTimeDelta);
}
