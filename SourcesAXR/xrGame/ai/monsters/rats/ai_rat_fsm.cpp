...
#define WRITE_TO_LOG(s) {\
	Msg("Monster %s : \n* State : %s\n* Time delta : %7.3f\n* Global time : %7.3f",*cName(),s,m_fTimeUpdateDelta,float(CRenderDevice::GetInstance()->dwTimeGlobal)/1000.f);\
...
void CAI_Rat::Death()
{
	...
	if (m_fFood <= 0) {
		...
		setVisible(false);
		if (CRenderDevice::GetInstance()->dwTimeGlobal - m_previous_query_time > 10000) {
//			setEnabled			(FALSE);
//			NET_Packet			P;
...
void CAI_Rat::UnderFire()
{
	...
	if (m_bStateChanged)//(CRenderDevice::GetInstance()->dwTimeGlobal - m_previous_query_time > TIME_TO_GO) || !m_previous_query_time)
...
void CAI_Rat::AttackRun()
{
	...
	CHECK_IF_GO_TO_NEW_STATE_THIS_UPDATE((memory().enemy().selected()->Position().distance_to(Position()) <= m_fAttackDistance) && (angle_difference(movement().m_body.target.yaw, sTemp.yaw) <= m_fAttackAngle),aiRatAttackFire)
		...
		if ((CRenderDevice::GetInstance()->dwTimeGlobal - m_previous_query_time > TIME_TO_GO) || !m_previous_query_time) {
...
void CAI_Rat::Retreat()
{
	...
	if	(!memory().enemy().selected() ||
		...
		|| 
		(
		(CRenderDevice::GetInstance()->dwTimeGlobal - memory().memory(memory().enemy().selected()).m_level_time > m_dwRetreatTime) && 
		(
		(m_tLastSound.dwTime < m_dwLastUpdateTime) || 
		!m_tLastSound.tpEntity || 
		(m_tLastSound.tpEntity->g_Team() == g_Team()) || 
...
void CAI_Rat::Pursuit()
{
	...
	if (memory().enemy().selected() && (CRenderDevice::GetInstance()->dwTimeGlobal - memory().memory(memory().enemy().selected()).m_level_time >= m_dwLostMemoryTime)) {
		memory().enable(memory().enemy().selected(),false);
...
void CAI_Rat::FreeRecoil()
{
	...
	if (m_bStateChanged) {
		m_dwLostRecoilTime	= CRenderDevice::GetInstance()->dwTimeGlobal;
	...
	CHECK_IF_GO_TO_NEW_STATE_THIS_UPDATE(memory().enemy().selected() && (CRenderDevice::GetInstance()->dwTimeGlobal - memory().memory(memory().enemy().selected()).m_level_time >= m_dwLostRecoilTime),aiRatPursuit);
...
void CAI_Rat::EatCorpse()
{
	...
	if (a && angle_difference(y,-movement().m_body.current.yaw) < PI_DIV_6) {
		m_fSpeed						= 0;
		if (CRenderDevice::GetInstance()->dwTimeGlobal - m_previous_query_time > m_dwHitInterval) {
			m_previous_query_time		= CRenderDevice::GetInstance()->dwTimeGlobal;
