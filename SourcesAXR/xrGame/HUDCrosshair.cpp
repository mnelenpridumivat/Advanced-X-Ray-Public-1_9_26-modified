...
{ 
	...
	CRenderDevice::GetInstance()->mProject.transform	(r,R);

	Fvector2		scr_size;
	scr_size.set	(static_cast<float>(::Render->getTarget()->get_width()), static_cast<float>(::Render->getTarget()->get_height()));
...
BOOL g_advanced_crosshair = FALSE;

void CHUDCrosshair::OnRender(const Fvector2& cent, const Fvector2& size_scr)
{
	VERIFY			(g_bRendering);
