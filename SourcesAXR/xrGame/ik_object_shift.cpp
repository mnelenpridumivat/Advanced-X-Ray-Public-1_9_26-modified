...
void object_shift::	dbg_draw			( const Fmatrix	&current_pos, const extrapolation::points &predict , const Fvector& start ) const
{
	...
	float time_global = CRenderDevice::GetInstance()->fTimeGlobal;
	...
	for( u16 i = 0; i < nb_points; ++i )
	{
		...
		Fvector	v0 = Fvector().add( p0, Fvector().mul( vadd, static_cast<float>(i) ) ).add( Fvector().set( 0, fshift0, 0 ) );
		Fvector	v1 = Fvector().add( p0, Fvector().mul( vadd, static_cast<float>(i + 1) ) ).add( Fvector().set( 0, fshift1, 0 ) );
...
void	object_shift::set_taget		( float taget_, float time )
{
	...
	if( time < EPS_S)
		time = CRenderDevice::GetInstance()->fTimeDelta;
