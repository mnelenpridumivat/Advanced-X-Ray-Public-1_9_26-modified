...
void CStateGroupAttackRunAbstract::initialize()
{
	...
	// interception
	m_intercept_tick = CRenderDevice::GetInstance()->dwTimeGlobal;
	...
	// prediction
	m_memorized_tick = CRenderDevice::GetInstance()->dwTimeGlobal;
	m_memorized_pos  = object->EnemyMan.get_enemy()->Position();
	m_predicted_vel  = cr_fvector3(0.f);

	// encirclement
	if (CRenderDevice::GetInstance()->dwTimeGlobal > m_next_encircle_tick )
	{
		m_encircle_time = 2000 + rand()%4000;
		m_next_encircle_tick = CRenderDevice::GetInstance()->dwTimeGlobal + 8000 + rand()%8000;
	}
	else
	{
		m_encircle_time = 0;	
...
{	
 	if (CRenderDevice::GetInstance()->dwTimeGlobal > m_intercept_tick + m_intercept_length )
 	{
 		m_intercept_tick   = CRenderDevice::GetInstance()->dwTimeGlobal;
 	...
 	if (CRenderDevice::GetInstance()->dwTimeGlobal > m_memorized_tick + memory_update_ms )
 	{
 		m_predicted_vel  = (enemy_pos-m_memorized_pos) * (1000.f / (CRenderDevice::GetInstance()->dwTimeGlobal-m_memorized_tick));
	...
	if ( !enemy_at_home || CRenderDevice::GetInstance()->dwTimeGlobal < time_state_started + m_encircle_time )
	{
...
// 
// 	//	if ( g_bDogsDirMode )
// 	if ( CRenderDevice::GetInstance()->dwTimeGlobal-time_state_started < m_max_encircle_time )
// 	{
//  		CMonsterSquad *squad	= monster_squad().get_squad(object);
//  		if (squad && squad->SquadActive()) {
//  			// Получить команду
