...
void	game_sv_mp::Update	()
{
	...
	for (u32 i=0; i<m_CorpseList.size(); )
	{
		...
		if (!pCorpseObj)
		{
			...
			Msg("corpse [%d] not found [%d]",CorpseID, CRenderDevice::GetInstance()->dwFrame);
			continue;
		}
		if (!pCorpseObj->children.empty())
		{
			Msg("corpse [%d] childern not empty [%d]",CorpseID, CRenderDevice::GetInstance()->dwFrame);
			i++;
			continue;
		}
		...
		m_CorpseList.erase(m_CorpseList.begin() + i);
		Msg("corpse [%d] send destroy [%d]",CorpseID, CRenderDevice::GetInstance()->dwFrame);
	}
	...
	{
		int curr_minutes = iFloor(CRenderDevice::GetInstance()->fTimeGlobal/60.0f);
...
void	game_sv_mp::SpawnPlayer(ClientID id, LPCSTR N)
{
	...
	if (pA) 
	{
		...
		ps_who->RespawnTime = CRenderDevice::GetInstance()->dwTimeGlobal;
...
#ifdef MP_LOGGING
	Msg("--- Player [%d] killed player [%d], Frame [%d]", KillerID, KilledID, CRenderDevice::GetInstance()->dwFrame);
#endif
...
//	R_ASSERT2( entity, "entity not found for destroying" );
	...
	NET_Packet P;
	P.w_begin(M_EVENT);
	P.w_u32(CRenderDevice::GetInstance()->dwTimeGlobal - 2*NET_Latency);
	P.w_u16(GE_DESTROY);
	P.w_u16(entity->ID);
	Level().Send(P, net_flags(TRUE,TRUE));
...
{
	if(!bRoundResult)
		ini.w_u32						(sect, "online_time_sec", CRenderDevice::GetInstance()->dwTimeGlobal/1000);
}
...
void game_sv_mp::DumpRoundStatisticsAsync()
{
	...
	m_async_stats_request_time = CRenderDevice::GetInstance()->dwTimeGlobal;
...
bool game_sv_mp::CheckStatisticsReady()
{
	...
	if (m_async_stats.all_ready() || 
		(m_async_stats_request_time + g_sv_dwMaxClientPing) < CRenderDevice::GetInstance()->dwTimeGlobal)
	{
		DumpRoundStatistics();
