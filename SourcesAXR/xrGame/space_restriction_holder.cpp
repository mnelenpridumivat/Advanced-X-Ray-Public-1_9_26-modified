...
shared_str CSpaceRestrictionHolder::normalize_string		(shared_str space_restrictors)
{
	...
	LPSTR					*strings = static_cast<LPSTR*>(_alloca(MAX_RESTRICTION_PER_TYPE_COUNT * sizeof(LPSTR)));
	LPSTR					*string_current = strings;

	LPSTR					temp_string = static_cast<LPSTR>(_alloca((n + 1) * sizeof(char)));
	...
	for ( ; *I; ++I, ++i) {
		...
		*i					= 0;
		VERIFY				(static_cast<u32>(string_current - strings) < MAX_RESTRICTION_PER_TYPE_COUNT);
		*string_current		= j;
		++string_current;
		j					= i + 1;
	}
	...
	*string_current			= j;
	...
	LPSTR					result_string = static_cast<LPSTR>(_alloca((n + 1) * sizeof(char)));
...
IC	void CSpaceRestrictionHolder::collect_garbage			()
{
	...
	for ( ; I != E; ) {
		if (!I->second->shape() && !I->second->m_ref_count && (CRenderDevice::GetInstance()->dwTimeGlobal >= I->second->m_last_time_dec + time_to_delete)) {
			J				= I;
			++I;
			xr_delete		(J->second);
