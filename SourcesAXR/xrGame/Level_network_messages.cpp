...
static bool SimmulateNetworkLag()
{
	...
	if (!max_lag_time || (max_lag_time <= CRenderDevice::GetInstance()->dwTimeGlobal))
	{
		CRandom				tmp_random(CRenderDevice::GetInstance()->dwTimeGlobal);
		max_lag_time		= CRenderDevice::GetInstance()->dwTimeGlobal + tmp_random.randI(lag_simmulator_min_ping, lag_simmulator_max_ping);
		return false;
	}
	return true;
}
