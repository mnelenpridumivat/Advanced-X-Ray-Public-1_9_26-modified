...
//		Msg							( "%6d stalker %s waits for the some door to be open/closed", CRenderDevice::GetInstance()->dwTimeGlobal, object().cName().c_str() );
		float						desirable_speed = old_desirable_speed();
		set_desirable_speed			(0.f);
...
//	Msg								( "%6d stalker %s is going", CRenderDevice::GetInstance()->dwTimeGlobal, object().cName().c_str() );
	...
	if (CRenderDevice::GetInstance()->dwTimeGlobal < (m_last_fail_time + fail_check_time)) {
		inherited::move_along_path	( movement_control, dest_position, time_delta);
		return;
	}
