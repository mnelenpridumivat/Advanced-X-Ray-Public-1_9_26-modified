...
{
//	Msg						("[%6d] enemy manager %s evaluates %s",CRenderDevice::GetInstance()->dwTimeGlobal,*m_object->cName(),*object->cName());
...
//	Msg							("%6d %s DEcreased enemy counter for player (%d -> %d)",CRenderDevice::GetInstance()->dwTimeGlobal,*m_object->cName(),Level().autosave_manager().not_ready_count(),Level().autosave_manager().not_ready_count()-1);
...
IC	bool CEnemyManager::enemy_inertia					(const CEntityAlive *previous_enemy) const
{
	if (smart_cast<CActor const*>(m_selected))
		return					(CRenderDevice::GetInstance()->dwTimeGlobal <= (m_last_enemy_change + ENEMY_INERTIA_TIME_TO_ACTOR));

	if (previous_enemy && smart_cast<CActor const*>(previous_enemy))
		return					(CRenderDevice::GetInstance()->dwTimeGlobal <= (m_last_enemy_change + ENEMY_INERTIA_TIME_FROM_ACTOR));

	return						(CRenderDevice::GetInstance()->dwTimeGlobal <= (m_last_enemy_change + ENEMY_INERTIA_TIME_TO_SOMEBODY));
}
...
void CEnemyManager::on_enemy_change						(const CEntityAlive *previous_enemy)
{
	...
	if (!previous_enemy->g_Alive()) {
		m_last_enemy_change		= CRenderDevice::GetInstance()->dwTimeGlobal;
		return;
	}
...
void CEnemyManager::process_wounded			(bool &only_wounded)
{
	...
	if (only_wounded) {
		...
		if (g_enemy_manager_second_update)
			Msg					("%6d ONLY WOUNDED LEFT %s", CRenderDevice::GetInstance()->dwTimeGlobal,*m_object->cName());
#endif // _DEBUG
		return;
	}
...
//		Msg						("%6d %s DEcreased enemy counter for player (%d -> %d)",CRenderDevice::GetInstance()->dwTimeGlobal,*m_object->cName(),Level().autosave_manager().not_ready_count(),Level().autosave_manager().not_ready_count()-1);
	...
	if (selected()) {
		m_last_enemy_time		= CRenderDevice::GetInstance()->dwTimeGlobal;
		m_last_enemy			= selected();
	}

	if (!m_ready_to_save) {
//		Msg						("%6d %s INcreased enemy counter for player (%d -> %d)",CRenderDevice::GetInstance()->dwTimeGlobal,*m_object->cName(),Level().autosave_manager().not_ready_count(),Level().autosave_manager().not_ready_count()+1);
	...
	if (g_enemy_manager_second_update && selected() && smart_cast<const CAI_Stalker*>(selected()) && smart_cast<const CAI_Stalker*>(selected())->wounded())
		Msg						("%6d WOUNDED CHOOSED %s", CRenderDevice::GetInstance()->dwTimeGlobal,*m_object->cName());
#endif // _DEBUG
