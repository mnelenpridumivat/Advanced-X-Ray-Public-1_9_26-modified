...
void CStalkerActionGetItemToKill::execute	()
{
	...
	VERIFY					((start_level_time() == CRenderDevice::GetInstance()->dwTimeGlobal) || (object().movement().mental_state() == eMentalStateDanger));
...
void CStalkerActionLookOut::initialize		()
{
	...
	if (CRenderDevice::GetInstance()->dwTimeGlobal >= m_last_change_time + CROUCH_LOOK_OUT_DELTA) {
		m_storage->set_property					(eWorldPropertyUseCrouchToLookOut,	!!m_crouch_look_out_random.random(2));
		m_last_change_time						= CRenderDevice::GetInstance()->dwTimeGlobal;
	}
...
void CStalkerCombatActionSmartCover::execute					()
{
	...
	if ( level_time + s_wait_enemy_in_smart_cover_time >= CRenderDevice::GetInstance()->dwTimeGlobal )
		return;
