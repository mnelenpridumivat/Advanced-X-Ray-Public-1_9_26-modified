...
void CRenderTarget::phase_ssao	()
{
	...
	// Compute params
	Fmatrix		m_v2w;			m_v2w.invert				(CRenderDevice::GetInstance()->mView		);

	float		fSSAONoise = 2.0f;
	fSSAONoise *= tan(deg2rad(67.5f));
	fSSAONoise /= tan(deg2rad(CRenderDevice::GetInstance()->fFOV));

	float		fSSAOKernelSize = 150.0f;
	fSSAOKernelSize *= tan(deg2rad(67.5f));
	fSSAOKernelSize /= tan(deg2rad(CRenderDevice::GetInstance()->fFOV));

	// Fill VB
	float	scale_X				= float(CRenderDevice::GetInstance()->dwWidth)	* 0.5f / float(TEX_jitter);
	float	scale_Y				= float(CRenderDevice::GetInstance()->dwHeight) * 0.5f / float(TEX_jitter);

	float _w = float(CRenderDevice::GetInstance()->dwWidth) * 0.5f;
	float _h = float(CRenderDevice::GetInstance()->dwHeight) * 0.5f;
	...
	set_viewport(HW.pContext, float(CRenderDevice::GetInstance()->dwWidth), float(CRenderDevice::GetInstance()->dwHeight));
...
void CRenderTarget::phase_downsamp	()
{
	...
	{
		set_viewport(HW.pContext, float(CRenderDevice::GetInstance()->dwWidth) * 0.5f, float(CRenderDevice::GetInstance()->dwHeight) * 0.5f);
		w /= 2;
		h /= 2;
	}
