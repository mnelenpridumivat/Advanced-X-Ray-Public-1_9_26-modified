...
{
	if (m_start_time > CRenderDevice::GetInstance()->dwTimeContinual - m_run_time)
	{

		Fvector2 pos = m_shniaga->GetWndPos();
		pos.y = this->pos(m_origin, m_destination, CRenderDevice::GetInstance()->dwTimeContinual - m_start_time);
		m_shniaga->SetWndPos(pos);		
	}else
		ProcessEvent(E_Stop);

	if (m_start_time > CRenderDevice::GetInstance()->dwTimeContinual - m_run_time*10/100)
		...
		case E_Begin:
			{
				...
				m_start_time = CRenderDevice::GetInstance()->dwTimeContinual;
