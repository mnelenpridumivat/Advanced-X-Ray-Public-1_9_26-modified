...
{
	if(CRenderDevice::GetInstance()->dwTimeGlobal - last_update <  extrapolate_pose_update_mseconds  )
		return;
	m_points.push_back( point().set( pose().set( m ), CRenderDevice::GetInstance()->fTimeGlobal ) );
	last_update = CRenderDevice::GetInstance()->dwTimeGlobal;
}

void	points::init	( const Fmatrix &m )
{
	m_points.fill_in( point().set( pose().set( m ), CRenderDevice::GetInstance()->fTimeGlobal ) );
	last_update = CRenderDevice::GetInstance()->dwTimeGlobal;
}
