...
{
	m_blink				= true;
	m_time_start_blink	= CRenderDevice::GetInstance()->dwTimeGlobal;
	m_time_last_blink	= 0;
}
...
void CInvisibility::frame_update()
{
	...
	if (!m_manual) {
		if (m_active)	m_energy -= m_speed * CRenderDevice::GetInstance()->fTimeDelta;
		else			m_energy += m_speed * CRenderDevice::GetInstance()->fTimeDelta;
