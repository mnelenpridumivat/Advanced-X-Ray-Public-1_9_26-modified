...
void CAgentExplosiveManager::register_explosive	(const CExplosive *explosive, const CGameObject *game_object)
{
	...
	m_explosives.push_back				(CDangerExplosive(explosive,game_object,0,CRenderDevice::GetInstance()->dwTimeGlobal));
	...
	object().location().add				(xr_new<CDangerObjectLocation>(game_object, CRenderDevice::GetInstance()->dwTimeGlobal,interval,GRENADE_RADIUS));
}
