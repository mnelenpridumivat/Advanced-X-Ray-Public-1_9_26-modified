...
void CScriptFcolor::script_register(lua_State *L)
{
	...
	[
		class_<Fcolor>("fcolor")
			...
			.def(								constructor<>())
			.def("set",							static_cast<Fcolor & (Fcolor::*)(float, float, float, float)>(&Fcolor::set),														return_reference_to<1>())
			.def("set",							static_cast<Fcolor & (Fcolor::*)(const Fcolor&)>(&Fcolor::set),																return_reference_to<1>())
