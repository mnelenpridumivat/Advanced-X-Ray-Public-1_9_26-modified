...
bool stalker_movement_manager_obstacles::simulate_path_navigation	()
{
	...
	while (!detail().completed(current_position,!detail().state_patrol_path(),current_travel_point)) {
		...
		if (!m_static_obstacles.process_query(false)) {
			m_last_fail_time		= CRenderDevice::GetInstance()->dwTimeGlobal;
...
//	Msg								("[%6d] m_failed_to_build_path = %s (stalker_movement_manager_obstacles::build_level_path)",CRenderDevice::GetInstance()->dwTimeGlobal,m_failed_to_build_path ? "true" : "false");
...
//	Msg								("[%6d][%6d][%s][%f] build_level_path",CRenderDevice::GetInstance()->dwFrame,CRenderDevice::GetInstance()->dwTimeGlobal,*object().cName(),timer.GetElapsed_sec()*1000.f);
}
