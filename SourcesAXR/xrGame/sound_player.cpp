...
void CSoundPlayer::update_playing_sounds()
{
	...
	for ( ; I != E; ++I) {
		if (I->m_sound->_feedback())
			...
			if (!I->started() && (CRenderDevice::GetInstance()->dwTimeGlobal >= I->m_start_time))
...
void CSoundPlayer::play				(u32 internal_type, u32 max_start_time, u32 min_start_time, u32 max_stop_time, u32 min_stop_time, u32 id)
{
	...
	sound_single.m_start_time	= CRenderDevice::GetInstance()->dwTimeGlobal + random_time;
	...
	if (CRenderDevice::GetInstance()->dwTimeGlobal >= m_playing_sounds.back().m_start_time)
