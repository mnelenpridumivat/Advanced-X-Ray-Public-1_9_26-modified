...
inline void CObjectList::o_crow		(CObject*	O)
{
	...
	O->dwFrame_AsCrow			= CRenderDevice::GetInstance()->dwFrame;
}
...
void CObject::MakeMeCrow			()
{
	...
	u32 const device_frame_id		= CRenderDevice::GetInstance()->dwFrame;
...
void	CObject::spatial_update		(float eps_P, float eps_R)
{
	...
	if (PositionStack.empty())
	{
		...
		PositionStack.back().dwTime		= CRenderDevice::GetInstance()->dwTimeGlobal;
...
void CObject::UpdateCL			()
{
	...
	if (CRenderDevice::GetInstance()->dwFrame==dbg_update_cl)								Debug.fatal	(DEBUG_INFO,"'UpdateCL' called twice per frame for %s",*cName());
	...
	{
		float dist = CRenderDevice::GetInstance()->vCameraPosition.distance_to_sqr(Position());
		if (dist < CROW_RADIUS*CROW_RADIUS)	
			MakeMeCrow	();
		else 
		if( (Visual() && Visual()->getVisData().hom_frame+2 > CRenderDevice::GetInstance()->dwFrame) && (dist < CROW_RADIUS2*CROW_RADIUS2) )
			MakeMeCrow	();
	}
}
...
void CObject::setDestroy			(BOOL _destroy)
{
	...
	if (_destroy)
	{
		...
		if(debug_destroy)
			Msg("cl setDestroy [%d][%d]",ID(), CRenderDevice::GetInstance()->dwFrame);
#endif
#ifdef MP_LOGGING
		Msg("cl setDestroy [%d][%d]",ID(), CRenderDevice::GetInstance()->dwFrame);
