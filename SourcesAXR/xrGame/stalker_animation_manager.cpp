...
{
	VERIFY						(fx_index >= 0);
	VERIFY						(fx_index < static_cast<int>(m_data_storage->m_part_animations.A[object().movement().body_state()].m_global.A[0].A.size()));
#ifdef DEBUG
	if (psAI_Flags.is(aiAnimation)) {
		...
		Msg						("%6d [%s][%s][%s][%f]", CRenderDevice::GetInstance()->dwTimeGlobal,*object().cName(),"FX",name,power_factor);
	}
#endif
