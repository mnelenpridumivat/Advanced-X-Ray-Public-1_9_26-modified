...
{
	fPhase+=FALL_SPEED* CRenderDevice::GetInstance()->fTimeDelta;
	if (fPhase<1)	
...
:CEffectorCam(eCEDOF, 100000)
{
	...
	m_fPhase						= CRenderDevice::GetInstance()->fTimeGlobal + dof.w;
}

BOOL CEffectorDOF::ProcessCam(SCamEffectorInfo& info)
{
	if (m_fPhase< CRenderDevice::GetInstance()->fTimeGlobal)
	{
