...
//lighting
	if(m_light_render->get_active()){
		...
		{
			int frame;
			u32 clr					= m_lanim->CalculateBGR(CRenderDevice::GetInstance()->fTimeGlobal,frame); // тючтЁр•рхЄ т ЇюЁьрЄх BGR
			Fcolor					fclr;
...
void CHelicopter::DieHelicopter()
{
	...
	if(CRenderDevice::GetInstance()->dwTimeGlobal != PositionStack.front().dwTime)
		lin_vel.div((CRenderDevice::GetInstance()->dwTimeGlobal-PositionStack.front().dwTime)/1000.0f);
