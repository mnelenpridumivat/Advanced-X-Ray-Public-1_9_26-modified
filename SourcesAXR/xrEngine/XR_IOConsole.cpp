...
void CConsole::Initialize()
{
	...
	m_cmd_history.erase(m_cmd_history.begin(), m_cmd_history.end());
	reset_cmd_history_idx();

	m_tips.reserve( MAX_TIPS_COUNT + 1 );
	m_tips.erase(m_tips.begin(), m_tips.end());
	m_temp_tips.reserve( MAX_TIPS_COUNT + 1 );
	m_temp_tips.erase(m_temp_tips.begin(), m_temp_tips.end());
...
void CConsole::DrawBackgrounds( bool bGame )
{
	...
	float tips_h    = _min<size_t>( m_tips.size(), (u32)VIEW_TIPS_COUNT ) * font_h;
...
{
	out_v.erase(out_v.begin(), out_v.end());
