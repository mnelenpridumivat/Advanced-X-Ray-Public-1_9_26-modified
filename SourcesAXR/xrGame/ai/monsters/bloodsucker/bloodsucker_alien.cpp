...
BOOL CAlienEffectorPP::Process(SPPInfo& pp)
{
	...
	def_lerp			(factor,target_factor, PERIOD_SPEED, CRenderDevice::GetInstance()->fTimeDelta);
...
BOOL CAlienEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	def_lerp(m_inertion, 1 - rel_dist, rel_dist, CRenderDevice::GetInstance()->fTimeDelta);
	...
	def_lerp(m_current_fov, m_target_fov, FOV_SPEED, CRenderDevice::GetInstance()->fTimeDelta);
