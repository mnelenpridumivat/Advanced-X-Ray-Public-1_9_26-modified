...
{
	if (!used() || (((m_last_query_time + m_query_interval) > CRenderDevice::GetInstance()->dwTimeGlobal) && !path_completed))
		return				(true);
...
IC	void CSelectorTemplate::perform_search		(const _vertex_id_type vertex_id)
{
	...
	m_last_query_time			= CRenderDevice::GetInstance()->dwTimeGlobal;
