...
void CStateMonsterHitObjectAbstract::execute()
{
	...
	if (!m_hitted && (time_state_started + TIME_POINTBREAK < CRenderDevice::GetInstance()->dwTimeGlobal)) {
		m_hitted		= true;
...
{
	if (time_state_started + TIME_OUT_STATE < CRenderDevice::GetInstance()->dwTimeGlobal) return true;
	return false;
}
