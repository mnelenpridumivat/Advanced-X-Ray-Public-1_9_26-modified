...
{
	return		(CRenderDevice::GetInstance());
}
#endif
...
struct profile_timer_script {
	...
	IC		float					time					() const
	{
		float					result = (static_cast<float>(double(m_accumulator) / double(CPU::clk_per_second))*1000000.f);
		return					(result);
	}
};
...
#ifdef XRGAME_EXPORTS
ICF	u32	script_time_global	()	{ return CRenderDevice::GetInstance()->dwTimeGlobal; }
ICF	u32	script_time_global_async	()	{ return CRenderDevice::GetInstance()->TimerAsync_MMT(); }
#else
