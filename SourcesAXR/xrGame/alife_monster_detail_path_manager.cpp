...
//	Msg								("[%6d][%s][%f][%f][%f]",CRenderDevice::GetInstance()->dwTimeGlobal,object().name_replace(),VPUSH(m_destination.m_position));
}
...
void CALifeMonsterDetailPathManager::target					(const CALifeSmartTerrainTask *task)
{
//	Msg								("[%6d][%s][%s]",CRenderDevice::GetInstance()->dwTimeGlobal,object().name_replace(),*task->patrol_path_name());
	target							(*task);
}
...
//		Msg							("[detail::update][%6d][%s]",CRenderDevice::GetInstance()->dwTimeGlobal,object().name_replace());
...
void CALifeMonsterDetailPathManager::follow_path				(const ALife::_TIME_ID &time_delta)
{
	...
	for ( ; m_path.size() > 1;) {
		...
		m_walked_distance				= 0.f;
		m_path.pop_back					();
//		Msg									("%6d %s changes graph point from %d to %d",CRenderDevice::GetInstance()->dwTimeGlobal,object().name_replace(),object().m_tGraphID,(GameGraph::_GRAPH_ID)m_path.back());
