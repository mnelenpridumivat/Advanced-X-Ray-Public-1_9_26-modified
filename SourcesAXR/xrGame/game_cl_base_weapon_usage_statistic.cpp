...
void WeaponUsageStatistic::OnPlayerSpawned(game_PlayerState* ps)
{
	...
	PlayerStat.last_alive_update_time	= CRenderDevice::GetInstance()->dwTimeGlobal;
}
		...
		void operator()(IClient* client)
		{
			...
			if (ps && !ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD))
			{
				if (xr_strlen(ps->getName()))
				{
					...
					u32 time_dif = CRenderDevice::GetInstance()->dwTimeGlobal - pstat.last_alive_update_time;
					pstat.m_dwTotalAliveTime[team]	+= time_dif;
					pstat.last_alive_update_time = CRenderDevice::GetInstance()->dwTimeGlobal;
				}
			}
		}
	};
