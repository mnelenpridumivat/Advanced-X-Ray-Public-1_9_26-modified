...
{
	float h1,p1;
	CRenderDevice::GetInstance()->vCameraDirection.getHP( h1, p1 );
...
{
	m_StartTime						= CRenderDevice::GetInstance()->fTimeGlobal;
...
{
	return ( (CRenderDevice::GetInstance()->fTimeGlobal - m_StartTime ) < m_lanim->Length_sec() );
}

void SHitMark::Draw( float cam_dir )
{
	int frame;
	u32 clr	= m_lanim->CalculateRGB(CRenderDevice::GetInstance()->fTimeGlobal - m_StartTime,frame );
...
SGrenadeMark::SGrenadeMark( const ui_shader& sh, CGrenade* grn )
{
	...
	m_LastTime						= CRenderDevice::GetInstance()->fTimeGlobal;
...
{
	return ( 2.0f*(CRenderDevice::GetInstance()->fTimeGlobal - m_LastTime ) < m_LightAnim->Length_sec() );
}

void SGrenadeMark::Draw( float cam_dir )
{
	int frame;
	u32 clr = m_LightAnim->CalculateRGB( 2.0f*(CRenderDevice::GetInstance()->fTimeGlobal - m_LastTime), frame );
	m_UIStaticItem->SetTextureColor( subst_alpha( m_UIStaticItem->GetTextureColor(), color_get_A(clr) ) );
