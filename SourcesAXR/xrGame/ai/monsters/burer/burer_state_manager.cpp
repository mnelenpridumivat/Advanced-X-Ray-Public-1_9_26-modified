...
void CStateManagerBurer::execute()
{
	...
	} else if (object->HitMemory.is_hit() && (object->HitMemory.get_last_hit_time() + 10000 > CRenderDevice::GetInstance()->dwTimeGlobal))
		state = eStateHitted;
	 else if (object->hear_interesting_sound)
		state = eStateHearInterestingSound;
	 else if (object->hear_dangerous_sound )
		state = eStateHearDangerousSound;
	 else if (object->time_last_scan + SCAN_STATE_TIME > CRenderDevice::GetInstance()->dwTimeGlobal)
