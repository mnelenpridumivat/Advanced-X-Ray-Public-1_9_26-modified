...
//				Msg				("[%6d][%s] actuality is false",CRenderDevice::GetInstance()->dwFrame,*object().cName());
...
{
//	Msg								("[%6d][%s][on_restrictions_change]",CRenderDevice::GetInstance()->dwTimeGlobal,*object().cName());
	m_path_actuality				= false;
	level_path_builder().remove		();
	detail_path_builder().remove	();
...
//	static int i=0;
//	Msg									("[%6d][%6d][%4d][%f] build_level_path",CRenderDevice::GetInstance()->dwTimeGlobal,CRenderDevice::GetInstance()->dwFrame,++i,timer.GetElapsed_sec()*1000.f);
}
