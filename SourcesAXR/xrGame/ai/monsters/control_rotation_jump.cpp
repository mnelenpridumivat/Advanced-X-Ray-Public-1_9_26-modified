...
void CControlRotationJump::on_release()
{
	...
	m_time_next_rotation_jump = CRenderDevice::GetInstance()->dwTimeGlobal + Random.randI(ROTATION_JUMP_DELAY_MIN,ROTATION_JUMP_DELAY_MAX);
}
...
bool CControlRotationJump::check_start_conditions()
{
	...
	if (m_time_next_rotation_jump > CRenderDevice::GetInstance()->dwTimeGlobal)	return false;
