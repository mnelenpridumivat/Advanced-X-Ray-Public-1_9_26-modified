...
void CParticlesPlayer::StartParticles(const shared_str& particles_name, u16 bone_num, const Fmatrix& xform, u16 sender_id, int life_time, bool auto_stop)
{
	...
	SBoneInfo* pBoneInfo			=  get_nearest_bone_info(reinterpret_cast<IKinematics*>(object->Visual()),bone_num);
	if(!pBoneInfo) return;
...
{
	VERIFY(pObject);
	IKinematics* pKinematics = reinterpret_cast<IKinematics*>(pObject->Visual()); VERIFY(pKinematics);
