...
bool game_cl_mp::OnKeyboardPress(int key)
{
	...
	if ( kJUMP == key || kWPN_FIRE == key )
	{
		...
		bool is_actor		= curr->IsA(CActor::StaticClass());
		bool is_spectator	= curr->IsA(CSpectator::StaticClass());
	...
	case KT_HIT:			//from hit
		{
			...
			if (pWeapon)
			{
				...
				if (pIItem)
				{
					...
					if (pIItem->IsA(CExplosiveItem::StaticClass()))
					{
			...
			case SKT_NONE:		// not special
				{
					if (pOKiller && pOKiller==Level().CurrentViewEntity())
					{
						...
						if ( pWeapon->IsA(CWeaponKnife::StaticClass()) )
						{
