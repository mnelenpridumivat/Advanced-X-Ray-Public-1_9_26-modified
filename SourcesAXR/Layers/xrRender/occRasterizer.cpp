	...
	for ( int i = 0; i< occ_dim_0; ++i)
	{
		for ( int j = 0; j< occ_dim_0; ++j)
		{
			if( bDebug )
			{
				...
				CRenderDevice::GetInstance()->mProject;

				float z = -CRenderDevice::GetInstance()->mProject._43/(float)(CRenderDevice::GetInstance()->mProject._33-quad.z);
				left_top.set		( quad.x*z/ CRenderDevice::GetInstance()->mProject._11/(occ_dim_0/2.f),		quad.y*z/ CRenderDevice::GetInstance()->mProject._22/(occ_dim_0/2.f), z);
				right_bottom.set	( (quad.x+1)*z/ CRenderDevice::GetInstance()->mProject._11/(occ_dim_0/2.f), (quad.y+1)*z/ CRenderDevice::GetInstance()->mProject._22/(occ_dim_0/2.f), z);
				...
				Fmatrix inv;
				inv.invert(CRenderDevice::GetInstance()->mView);
