...
inline xr_unique_ptr<T>&& xr_make_unique(Args&&... args)
{
    static_assert(std::is_base_of<T, A>());
    return xr_unique_ptr<T>(xr_new<A>(std::forward<Args>(args)...));
}
...
{
    static_assert(std::is_base_of<T, A>());
    return xr_shared_ptr<T>(xr_new<A>(std::forward<Args>(args)...), xr_custom_deleter<A>());
}
