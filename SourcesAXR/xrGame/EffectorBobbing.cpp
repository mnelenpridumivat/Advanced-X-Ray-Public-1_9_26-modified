...
{
	fTime			+= CRenderDevice::GetInstance()->fTimeDelta;
	if (dwMState&ACTOR_DEFS::mcAnyMove){
		if (fReminderFactor<1.f)	fReminderFactor += SPEED_REMINDER* CRenderDevice::GetInstance()->fTimeDelta;
		else						fReminderFactor = 1.f;
	}else{
		if (fReminderFactor>0.f)	fReminderFactor -= SPEED_REMINDER* CRenderDevice::GetInstance()->fTimeDelta;
