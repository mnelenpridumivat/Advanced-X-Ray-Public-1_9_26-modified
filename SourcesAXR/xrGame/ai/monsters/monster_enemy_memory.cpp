...
void CMonsterEnemyMemory::update() 
{
	...
	if ( monster->SoundMemory.IsRememberSound() )
	{
		...
		if ( dangerous && CRenderDevice::GetInstance()->dwTimeGlobal < sound.time + 2000 )
		{
	...
	for (ENEMIES_MAP_IT it = m_objects.begin(); it != m_objects.end(); it++) {
		u8		relation_value = static_cast<u8>(monster->tfGetRelationType(it->first));
...
void CMonsterEnemyMemory::add_enemy(const CEntityAlive *enemy)
{
	...
	enemy_info.time		= CRenderDevice::GetInstance()->dwTimeGlobal;
...
{
	TTime cur_time = CRenderDevice::GetInstance()->dwTimeGlobal;
