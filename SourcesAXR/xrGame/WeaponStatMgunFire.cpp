...
void CWeaponStatMgun::UpdateFire()
{
	...
	{
		angle_lerp		(m_dAngle.x,0.f,5.f, CRenderDevice::GetInstance()->fTimeDelta);
		angle_lerp		(m_dAngle.y,0.f,5.f, CRenderDevice::GetInstance()->fTimeDelta);
	}
}
...
void CWeaponStatMgun::AddShotEffector				()
{
	if(OwnerActor())
	{
		...
		if (!S)	S			= static_cast<CCameraShotEffector*>(OwnerActor()->Cameras().AddCamEffector(xr_new<CCameraShotEffector>(camera_recoil)));
		R_ASSERT			(S);
