...
//		Msg					("[%6d][%s] actuality is false 2",CRenderDevice::GetInstance()->dwFrame,*object().cName());
	...
	switch (m_path_state) {
		...
		case ePathStatePathVerification : {
			if (!patrol().actual()) {
//				Msg				("[%6d][%s] actuality is false 3",CRenderDevice::GetInstance()->dwFrame,*object().cName());
				m_path_state	= ePathStateSelectPatrolPoint;
			}
			else
...
//							Msg				("[%6d][%s] detail path is completed",CRenderDevice::GetInstance()->dwFrame,*object().cName());
//							Msg				(
...
//							);
		...
		case ePathStatePathCompleted : {
			if (!patrol().actual()) {
//				Msg				("[%6d][%s] actuality is false 4",CRenderDevice::GetInstance()->dwFrame,*object().cName());
				m_path_state	= ePathStateSelectPatrolPoint;
			}
			break;
		}
