	...
	switch ( m_action )
	{
		case ACTION_TELE_STARTED:
			...
			if ( !time_started )
			{
				...
				time_started					= CRenderDevice::GetInstance()->dwTimeGlobal;
			}
			else
			{
...
{
	if (time_started + object->m_tele_time_to_hold > CRenderDevice::GetInstance()->dwTimeGlobal) return;
	...
	for (u32 i = 0; i < this->object->CTelekinesis::get_objects_total_count(); ++i)
	{
		...
		if ((tele_object.get_state() == TS_Keep) && (tele_object.time_keep_started + 1500 < CRenderDevice::GetInstance()->dwTimeGlobal))
		{
	...
	if (object_found) {
		...
		if (!IsActiveObjects() || (time_started + MAX_TIME_CHECK_FAILURE < CRenderDevice::GetInstance()->dwTimeGlobal)) {
			m_action						= ACTION_COMPLETED;
		} 
	}
}
