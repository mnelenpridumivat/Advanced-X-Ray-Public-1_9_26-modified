...
void	smapvis::invalidate	()
{
	...
	testQ_V		=	0;
	frame_sleep	= CRenderDevice::GetInstance()->dwFrame + ps_r__LightSleepFrames;
	invisible.clear	();
}
void	smapvis::begin		()
{
...
void	smapvis::end		()
{
	...
	case state_working:
		...
		if (testQ_V)
		{
			...
			testQ_frame								= CRenderDevice::GetInstance()->dwFrame + 1;	// get result on next frame
		}
		break;
	case state_usingTC:
		// nothing to do
...
{
	// the tough part
	if (testQ_frame != CRenderDevice::GetInstance()->dwFrame)			return;
...
{
	if (testQ_frame==(CRenderDevice::GetInstance()->dwFrame+1))		testQ_frame--;
	flushoccq		();
}
