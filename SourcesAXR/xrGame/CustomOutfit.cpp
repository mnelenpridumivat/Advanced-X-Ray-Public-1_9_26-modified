...
void CCustomOutfit::UpdateFilterCondition(void)
{
	...
	if (outfit && m_bUseFilter)
	{
		...
		float uncharge_coef = ((m_fFilterDegradation + m_radia_hit + m_chemical_hit) / 16) * CRenderDevice::GetInstance()->fTimeDelta;
...
//	m_HitTypeProtection[ALife::eHitTypePhysicStrike]= pSettings->r_float(section,"physic_strike_protection");
	...
	clamp( m_artefact_count, static_cast<u32>(0), static_cast<u32>(GameConstants::GetArtefactsCount()));
...
{
	return m_boneProtection->getBonePassBullet(static_cast<s16>(boneID));
}
