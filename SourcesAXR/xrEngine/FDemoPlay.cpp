...
CDemoPlay::CDemoPlay(const char *name, float ms, u32 cycles, float life_time) : CEffectorCam(cefDemo,life_time/*,FALSE*/)
{
	...
	stat_started		= FALSE;
	CRenderDevice::GetInstance()->PreCache		(50, true, false);
}
...
void CDemoPlay::stat_Start	()
{
	...
	stat_started			= TRUE				;
	Sleep					(1)					;
	stat_StartFrame			= CRenderDevice::GetInstance()->dwFrame	;
...
void CDemoPlay::stat_Stop	()
{
	...
	// total
	u32	dwFramesTotal		= CRenderDevice::GetInstance()->dwFrame-stat_StartFrame	;
...
BOOL CDemoPlay::ProcessCam(SCamEffectorInfo& info)
{
	...
	if (CRenderDevice::GetInstance()->dwPrecacheFrame)	return	TRUE;
	...
	if (m_pMotion)
	{
		...
		m_MParam->Update		(CRenderDevice::GetInstance()->fTimeDelta,1.f,true);
	...
	else
	{
		...
		fStartTime		+= CRenderDevice::GetInstance()->fTimeDelta;
		...
		for (int i=0; i<4; i++) {
			...
			spline1	( t, &(v[0]), (Fvector *) &(CRenderDevice::GetInstance()->mView.m[i][0]) );
		}
