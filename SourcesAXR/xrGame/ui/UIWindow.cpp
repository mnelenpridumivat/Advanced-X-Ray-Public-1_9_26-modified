...
{	
	...
	if( WINDOW_LBUTTON_DOWN == mouse_action )
	{
		...
		u32 dwCurTime						= CRenderDevice::GetInstance()->dwTimeContinual;

		if( (_last_db_click_frame!= CRenderDevice::GetInstance()->dwFrame) && (dwCurTime-m_dwLastClickTime < DOUBLE_CLICK_TIME) )
		{
            mouse_action			= WINDOW_LBUTTON_DB_CLICK;
			_last_db_click_frame	= CRenderDevice::GetInstance()->dwFrame;
		}
...
{
	m_dwFocusReceiveTime	= CRenderDevice::GetInstance()->dwTimeGlobal;
