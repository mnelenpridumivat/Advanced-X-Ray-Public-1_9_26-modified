...
void dxThunderboltRender::Render(CEffect_Thunderbolt &owner)
{
	...
	// top
	{
		...
		vecSx.mul		(CRenderDevice::GetInstance()->vCameraRight, 	owner.current->m_GradientTop->fRadius.x*owner.lightning_size);
		vecSy.mul		(CRenderDevice::GetInstance()->vCameraTop, 	-owner.current->m_GradientTop->fRadius.y*owner.lightning_size);
	...
	// center
	{
		u32 c_val		= iFloor(owner.current->m_GradientTop->fOpacity*owner.lightning_phase*255.f);
		u32 c			= color_rgba(c_val,c_val,c_val,c_val);
		vecSx.mul		(CRenderDevice::GetInstance()->vCameraRight, 	owner.current->m_GradientCenter->fRadius.x*owner.lightning_size);
		vecSy.mul		(CRenderDevice::GetInstance()->vCameraTop, 	-owner.current->m_GradientCenter->fRadius.y*owner.lightning_size);
