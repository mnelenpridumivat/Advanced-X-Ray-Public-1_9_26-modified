...
{
		s.calc_time			= CRenderDevice::GetInstance()->dwTimeGlobal;
...
IC bool state_valide( const calculate_state &prev_state )
{
	...
	if( dbg_always_valide )
		return	true;
#endif
	return (CRenderDevice::GetInstance()->dwTimeGlobal <= (prev_state.calc_time + UCalc_Interval + CRenderDevice::GetInstance()->dwTimeDelta));
}
