...
void SBoneProtections::reload(const shared_str& bone_sect, IKinematics* kinematics)
{
	...
	for (CInifile::SectCIt i=protections.Data.begin(); protections.Data.end() != i; ++i) 
	{
		...
		BP.koeff			= static_cast<float>(atof(_GetItem(i->second.c_str(), 0, buffer)));
		BP.armor			= static_cast<float>(atof(_GetItem(i->second.c_str(), 1, buffer)));
		BP.BonePassBullet	= static_cast<BOOL>(atof(_GetItem(i->second.c_str(), 2, buffer)) > 0.5f);
