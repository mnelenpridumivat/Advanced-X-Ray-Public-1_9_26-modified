...
CSE_ALifeInventoryItem::CSE_ALifeInventoryItem(LPCSTR caSection)
{
	...
	State.linear_vel.set		(0.f,0.f,0.f);
			...
			m_freeze_time	= CRenderDevice::GetInstance()->dwTimeGlobal;
#else
			m_freeze_time	= 0;
#endif
...
#ifdef XRGAME_EXPORTS
	if (CRenderDevice::GetInstance()->dwTimeGlobal >= (m_freeze_time + m_freeze_delta_time))
		return		FALSE;
#endif
...
CSE_ALifeItemWeapon::CSE_ALifeItemWeapon	(LPCSTR caSection) : CSE_ALifeItem(caSection)
{
	...
	m_laser_designator_status	= static_cast<EWeaponAddonStatus>(READ_IF_EXISTS(pSettings, r_s32, s_name, "laser_designator_status", 0));
	m_tactical_torch_status		= static_cast<EWeaponAddonStatus>(READ_IF_EXISTS(pSettings, r_s32, s_name, "tactical_torch_status", 0));
