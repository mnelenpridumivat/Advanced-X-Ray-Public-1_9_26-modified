...
//	R_ASSERT2( e_entity, make_string( "entity not found. parent_id = [%d], entity_id = [%d], frame = [%d]", id_parent, id_entity, CRenderDevice::GetInstance()->dwFrame ).c_str() );
	VERIFY2  ( e_entity, make_string( "entity not found. parent_id = [%d], entity_id = [%d], frame = [%d]", id_parent, id_entity, CRenderDevice::GetInstance()->dwFrame ).c_str() );
	if ( !e_entity ) {
		Msg                ( "! ERROR on rejecting: entity not found. parent_id = [%d], entity_id = [%d], frame = [%d].", id_parent, id_entity, CRenderDevice::GetInstance()->dwFrame );
		return false;
	}

//	R_ASSERT2( e_parent, make_string( "parent not found. parent_id = [%d], entity_id = [%d], frame = [%d]", id_parent, id_entity, CRenderDevice::GetInstance()->dwFrame ).c_str() );
	VERIFY2  ( e_parent, make_string( "parent not found. parent_id = [%d], entity_id = [%d], frame = [%d]", id_parent, id_entity, CRenderDevice::GetInstance()->dwFrame ).c_str() );
	if ( !e_parent ) {
		Msg                ( "! ERROR on rejecting: parent not found. parent_id = [%d], entity_id = [%d], frame = [%d].", id_parent, id_entity, CRenderDevice::GetInstance()->dwFrame );
		return false;
	}
...
//.	Msg("---ID_Parent [%d], id_parent [%d]", e_entity->ID_Parent, id_parent);
	...
	if (e_entity->ID_Parent != id_parent)
	{
		Msg("! ERROR: e_entity->ID_Parent = [%d]  parent = [%d][%s]  entity_id = [%d]  frame = [%d]",
			e_entity->ID_Parent, id_parent, e_parent->name_replace(), id_entity, CRenderDevice::GetInstance()->dwFrame);
		//it can't be !!!
	}
