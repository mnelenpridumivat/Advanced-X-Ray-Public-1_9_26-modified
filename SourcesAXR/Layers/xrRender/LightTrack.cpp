...
void	CROS_impl::add		(light* source)
{
	...
	for (xr_vector<Item>::iterator I=track.begin(); I!=track.end(); I++)	
		if (source == I->source)	{ I->frame_touched = CRenderDevice::GetInstance()->dwFrame; return; }

	// Register _new_
	track.push_back		(Item());
	Item&	L			= track.back();
	L.frame_touched		= CRenderDevice::GetInstance()->dwFrame;
	L.source			= source;
...
{
	if (dwFrameSmooth == CRenderDevice::GetInstance()->dwFrame)
		return;

	dwFrameSmooth			= CRenderDevice::GetInstance()->dwFrame;
	...
	float	l_f				= CRenderDevice::GetInstance()->fTimeDelta*ps_r2_lt_smooth;
	clamp	(l_f,0.f,1.f)	;
	float	l_i				=	1.f-l_f							;
...
void CROS_impl::prepare_lights(Fvector& position, IRenderable* O)
{
	...
	if		(bTraceLights)	{
		...
		for (s32 id=0; id<s32(track.size()); id++)
		{
			...
			if (I->frame_touched!= CRenderDevice::GetInstance()->dwFrame)	{ track.erase(I) ; id--	; continue ; }
