...
{
	if (Phase()!=GAME_PHASE_INPROGRESS) return false;
	if (Level().CurrentEntity() && !Level().CurrentEntity()->IsA(CActor::StaticClass()))
	{
		return FALSE;
	}
	if (pCurSkinMenu && pCurSkinMenu->IsShown())
	...
	case GAME_PHASE_INPROGRESS:
		{
			...
			if(local_player && !local_player->IsSkip())
			{
				...
				if (Level().CurrentControlEntity() && 
					Level().CurrentControlEntity()->IsA(CSpectator::StaticClass()) &&
...
bool	game_cl_Deathmatch::OnKeyboardPress			(int key)
{
	...
	if (kINVENTORY == key )
	{
		if (Level().CurrentControlEntity() && Level().CurrentControlEntity()->IsA(CActor::StaticClass()))
		{
			if (m_game_ui)
			{
...
void game_cl_Deathmatch::OnSpawn(CObject* pObj)
{
	...
	if (pObj->IsA(CWeapon::StaticClass()))
	{
		if (pObj->H_Parent())
		{
