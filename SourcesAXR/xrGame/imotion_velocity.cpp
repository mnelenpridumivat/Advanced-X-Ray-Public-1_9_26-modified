...
void	imotion_velocity::state_end(  )
{
	VERIFY( shell );
	inherited::state_end( );
	shell->AnimToVelocityState(CRenderDevice::GetInstance()->fTimeDelta, default_l_limit * 10, default_w_limit * 10 );
...
{
		VERIFY( shell );
		if( !shell->AnimToVelocityState(CRenderDevice::GetInstance()->fTimeDelta, 2 * default_l_limit, 10.f * default_w_limit ) )
