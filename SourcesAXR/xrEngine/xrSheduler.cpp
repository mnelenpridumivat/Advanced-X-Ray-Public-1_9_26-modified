...
void CSheduler::internal_Register	(ISheduled* O, BOOL RT)
{
	...
	if (RT)
	{
		// Fill item structure
		Item						TNext;
		TNext.dwTimeForExecute		= CRenderDevice::GetInstance()->dwTimeGlobal;
		TNext.dwTimeOfLastExecute	= CRenderDevice::GetInstance()->dwTimeGlobal;
		TNext.Object				= O;
...
#ifdef DEBUG
		T.Object->dbg_startframe	= CRenderDevice::GetInstance()->dwFrame;
		eTimer.Start				();
...
//		}
		...
		if (CRenderDevice::GetInstance()->dwPrecacheFrame==0 && CPU::QPC() > cycles_limit)
		{
...
{
	R_ASSERT						(CRenderDevice::GetInstance()->Statistic);
	// Initialize
	CRenderDevice::GetInstance()->Statistic->Sheduler.Begin();
...
#ifdef DEBUG_SCHEDULER
	Msg								("SCHEDULER: PROCESS STEP %d", CRenderDevice::GetInstance()->dwFrame);
#endif // DEBUG_SCHEDULER
	// Realtime priority
	...
	for (u32 it=0; it<ItemsRT.size(); it++)
	{
		...
		u32	Elapsed					= dwTime-T.dwTimeOfLastExecute;
#ifdef DEBUG
		VERIFY						(T.Object->dbg_startframe != CRenderDevice::GetInstance()->dwFrame);
		T.Object->dbg_startframe	= CRenderDevice::GetInstance()->dwFrame;
#endif
	...
	m_processing_now				= false;
#ifdef DEBUG_SCHEDULER
	Msg								("SCHEDULER: PROCESS STEP FINISHED %d", CRenderDevice::GetInstance()->dwFrame);
#endif // DEBUG_SCHEDULER
	clamp							(psShedulerTarget,3.f,66.f);
	psShedulerCurrent				= 0.9f*psShedulerCurrent + 0.1f*psShedulerTarget;
	CRenderDevice::GetInstance()->Statistic->fShedulerLoad	= psShedulerCurrent;

	// Finalize
	g_bSheduleInProgress			= FALSE;
	internal_Registration			();
	CRenderDevice::GetInstance()->Statistic->Sheduler.End	();
}
