...
void CEffect_Thunderbolt::Bolt(shared_str id, float period, float lt)
{
    ...
    pos.mad		            (CRenderDevice::GetInstance()->vCameraPosition,current_direction,dist);
    ...
    if (next_v<environment.p_second_prop){
	    next_lightning_time = CRenderDevice::GetInstance()->fTimeGlobal+lt+EPS_L;
    }else{
	    next_lightning_time = CRenderDevice::GetInstance()->fTimeGlobal+period+Random.randF(-period*0.3f,period*0.3f);
...
void CEffect_Thunderbolt::OnFrame(shared_str id, float period, float duration)
{
	...
	if (bEnabled!=enabled){
    	bEnabled			= enabled;
	    next_lightning_time = CRenderDevice::GetInstance()->fTimeGlobal+period+Random.randF(-period*0.5f,period*0.5f);
    }else if (bEnabled&&(CRenderDevice::GetInstance()->fTimeGlobal>next_lightning_time)){
    	if (state==stIdle && !!(id.size())) Bolt(id,period,duration);
    }
	if (state==stWorking){
    	if (current_time>life_time) state = stIdle;
    	current_time	+= CRenderDevice::GetInstance()->fTimeDelta;
		Fvector fClr;		
		int frame;
