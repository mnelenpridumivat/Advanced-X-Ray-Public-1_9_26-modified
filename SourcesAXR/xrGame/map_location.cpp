...
CMapLocation::CMapLocation(LPCSTR type, u16 object_id)
{
	...
	m_cached.m_updatedFrame = static_cast<u32>(-1);
	m_cached.m_graphID		= static_cast<GameGraph::_GRAPH_ID>(-1);
	if(!IsGameTypeSingle())
...
void CMapLocation::LoadSpot(LPCSTR type, bool bReload)
{
	...
	if ( m_ttl > 0 )
	{
		...
		m_actual_time = CRenderDevice::GetInstance()->dwTimeGlobal+m_ttl*1000;
	}
...
const Fvector2& CMapLocation::CalcDirection()
{
	...
	{
		m_cached.m_Direction.set(CRenderDevice::GetInstance()->vCameraDirection.x, CRenderDevice::GetInstance()->vCameraDirection.z);
	}else
	{
		CObject* pObject =  Level().Objects.net_Find(m_objectID);
		if(!pObject)
	...
	if(m_flags.test(ePosToActor)){
		...
		if(pObject){
			...
			dcp.set(CRenderDevice::GetInstance()->vCameraPosition.x, CRenderDevice::GetInstance()->vCameraPosition.z);
...
{
	R_ASSERT(m_cached.m_updatedFrame!= CRenderDevice::GetInstance()->dwFrame);
		

	if(	m_flags.test(eTTL) )
	{
		if( m_actual_time < CRenderDevice::GetInstance()->dwTimeGlobal)
		{
			m_cached.m_Actuality		= false;
			m_cached.m_updatedFrame		= CRenderDevice::GetInstance()->dwFrame;
