...
/*			CHudItem* pHudItem = smart_cast<CHudItem*>(pActor->inventory().ActiveItem());
	...
	if (CRenderDevice::GetInstance()->Paused() && pOldActor)
	{
#ifdef DEBUG
...
void CSpectator::cam_Update	(CActor* A)
{
	if (A){
		...
		if (CRenderDevice::GetInstance()->Paused())
		{
			CRenderDevice::GetInstance()->fTimeDelta = m_fTimeDelta;	//fake, to update cam (problem with fov)
			g_pGameLevel->Cameras().UpdateFromCamera(cam);
			CRenderDevice::GetInstance()->fTimeDelta = 0.0f;			//fake, to update cam (problem with fov)
		} else
		{
			g_pGameLevel->Cameras().UpdateFromCamera(cam);
		}
