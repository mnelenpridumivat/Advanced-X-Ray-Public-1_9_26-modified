...
void CSightAction::initialize					()
{
	...
	m_state_fire_switch_time		= CRenderDevice::GetInstance()->dwTimeGlobal;
...
#ifdef SIGHT_TEST
	Msg					("%6d eSightTypeCurrentDirection",CRenderDevice::GetInstance()->dwTimeGlobal);
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d eSightTypePathDirection",CRenderDevice::GetInstance()->dwTimeGlobal);
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d eSightTypeDirection",CRenderDevice::GetInstance()->dwTimeGlobal);
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d %s",CRenderDevice::GetInstance()->dwTimeGlobal,m_torso_look ? "eSightTypeFirePosition" : "eSightTypePosition");
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d %s",CRenderDevice::GetInstance()->dwTimeGlobal,m_torso_look ? "eSightTypeFireObject" : "eSightTypeObject");
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d %s [%f] -> [%f]",CRenderDevice::GetInstance()->dwTimeGlobal,m_torso_look ? "eSightTypeFireCover" : "eSightTypeCover",object().movement().m_body.current.yaw,object().movement().m_body.target.yaw);
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d %s", CRenderDevice::GetInstance()->dwTimeGlobal,m_torso_look ? "eSightTypeFireSearch" : "eSightTypeSearch");
#endif
}
...
{
	m_internal_state	= 2;
	m_start_state_time	= CRenderDevice::GetInstance()->dwTimeGlobal;
...
fall_back:
#endif // #ifndef DEBUG
			if ((m_start_state_time + m_stop_state_time < CRenderDevice::GetInstance()->dwTimeGlobal) && target_reached()) {
				m_start_state_time	= CRenderDevice::GetInstance()->dwTimeGlobal;
				m_stop_state_time	= 3500;
				m_internal_state	= 1;
...
void CSightAction::predict_object_position		( bool use_exact_position )
{
	...
	if ( count > 1 ) {
		...
		VERIFY					(CRenderDevice::GetInstance()->dwTimeGlobal >= current_position.dwTime );
		...
		if (CRenderDevice::GetInstance()->dwTimeGlobal - previous_position.dwTime < 300 ) {
			if ( current_position.dwTime > previous_position.dwTime ) {
				...
				float const predict_time	= g_aim_predict_time;//*CRenderDevice::GetInstance()->fTimeDelta;
...
//			Msg							("%6d switch to mode 1", CRenderDevice::GetInstance()->dwTimeGlobal);
...
//			m_vector3d					= m_object->sight().object_position();
		...
		case 1 : {
			...
			if (CRenderDevice::GetInstance()->dwTimeGlobal >= m_state_fire_switch_time + 1500 ) {
				if (m_object_to_look->Position().distance_to_sqr(m_object->Position()) > _sqr(5.f)) {
					if (!m_holder_start_position.similar(m_object->Position(),.05f)) {
						m_vector3d			= m_object->sight().object_position();
						m_already_switched	= false;
//						Msg					("%6d switch to mode 0 (reson: holder position changed)", CRenderDevice::GetInstance()->dwTimeGlobal);
						m_state_fire_object	= 0;
						break;
					}
					...
					if (!m_object_start_position.similar(m_object_to_look->Position(),.05f)) {
						m_vector3d			= m_object->sight().object_position();
//						Msg					("%6d switch to mode 0 (reson: object position changed)", CRenderDevice::GetInstance()->dwTimeGlobal);
						m_already_switched	= false;
						m_state_fire_object	= 0;
						break;
					}
				...
				if ( !m_already_switched) {
					m_vector3d				= m_object->sight().object_position();
//					Msg						("%6d switch to mode 0 (reson: time interval)", CRenderDevice::GetInstance()->dwTimeGlobal);
