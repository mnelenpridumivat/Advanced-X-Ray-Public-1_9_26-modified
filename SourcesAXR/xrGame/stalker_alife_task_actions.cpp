...
#ifndef GRENADE_TEST
	m_stop_weapon_handling_time					= CRenderDevice::GetInstance()->dwTimeGlobal;
...
#ifndef GRENADE_TEST
	if (CRenderDevice::GetInstance()->dwTimeGlobal >= m_stop_weapon_handling_time)
...
//			Msg										("weapon %s is strapped : %c",*weapon->cName(),weapon->strapped_mode() ? '+' : '-');
			...
			if (!object().inventory().ActiveItem() || (object().inventory().GetActiveSlot() == INV_SLOT_2)) {
				if (!m_time_to_strap)
					m_time_to_strap					= CRenderDevice::GetInstance()->dwTimeGlobal + 10000;
				if (CRenderDevice::GetInstance()->dwTimeGlobal >= m_time_to_strap) {
					m_time_to_idle					= 0;
					object().CObjectHandler::set_goal	(eObjectActionStrapped,object().best_weapon());
				}
			}
				...
				if (weapon->strapped_mode()) {
					if (!m_time_to_idle)
						m_time_to_idle					= CRenderDevice::GetInstance()->dwTimeGlobal + 10000;
					if (CRenderDevice::GetInstance()->dwTimeGlobal >= m_time_to_idle) {
						m_time_to_strap					= 0;
