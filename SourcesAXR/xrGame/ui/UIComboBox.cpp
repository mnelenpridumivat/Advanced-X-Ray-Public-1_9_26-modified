...
CUIListBoxItem* CUIComboBox::AddItem_(LPCSTR str, int _data)
{
	...
	itm->SetData		((void*)static_cast<__int64>(_data));
	return				itm;
}
...
void CUIComboBox::OnListItemSelect()
{
	...
	m_itoken_id				= static_cast<int>((__int64)itm->GetData());
	ShowList				(false);
...
{
	if (static_cast<u32>(index) >= GetSize())
		return "";
...
void CUIComboBox::Update()
{
	...
	else
	{
		...
		if(m_list_frame.IsShown())
		{
			...
			CRenderDevice::GetInstance()->seqRender.Add		(this, 3);
		}
	}
