...
void CMonsterSoundMemory::UpdateHearing()
{
	...
	Sounds.erase	(
		std::remove_if(
			...
			pred_remove_nonactual_sounds(
				CRenderDevice::GetInstance()->dwTimeGlobal - time_memory
			)
		),
		Sounds.end()
	);
	...
	// пересчитать value
	for (xr_vector<SoundElem>::iterator I = Sounds.begin(); I != Sounds.end(); ++I) I->CalcValue(CRenderDevice::GetInstance()->dwTimeGlobal, monster->Position());
