...
void CLight_Compute_XFORM_and_VIS::compute_xf_spot	(light* L)
{
	...
	float	dist				= CRenderDevice::GetInstance()->vCameraPosition.distance_to(L->spatial.sphere.P)-L->spatial.sphere.R;
			if (dist<0)	dist	= 0;
	...
	float	duel_dot			= 1.f -	0.5f* CRenderDevice::GetInstance()->vCameraDirection.dotproduct(L_dir);
