...
void CActor::net_Export	(NET_Packet& P)					// export to server
{
	...
	P.w_float /*w_angle8*/			(angle_normalize(r_model_yaw)); //CRenderDevice::GetInstance()->vCameraDirection.getH());//
...
/*	
	if (OnClient())
	{
		if (!pStatGraph)
		{
			...
			pStatGraph->SetRect(0, g_Y, CRenderDevice::GetInstance()->dwWidth, 100, 0xff000000, 0xff000000);
			g_Y += 110;
			if (g_Y > 700) g_Y = 100;
