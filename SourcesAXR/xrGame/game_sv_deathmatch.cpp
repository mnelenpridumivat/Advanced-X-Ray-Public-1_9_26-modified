...
BOOL	game_sv_Deathmatch::OnTouch			(u16 eid_who, u16 eid_what, BOOL bForced)
{
	...
	if (A)
	{
		...
		if (e_what->IsA(CSE_ALifeItemAmmo::StaticClass())
			|| e_what->IsA(CSE_ALifeItemGrenade::StaticClass())
			|| e_what->IsA(CSE_ALifeItemCustomOutfit::StaticClass())) 
		{
			return TRUE;
		}
...
{	
	...
	if (e_entity->m_tClassID == CLSID_OBJECT_PLAYERS_BAG && actor)
	{
		...
		for ( ;it!=it_e; ++it)
		{
			...
			{
				if (!e_item->IsA(CSE_ALifeItemCustomOutfit::StaticClass()))
