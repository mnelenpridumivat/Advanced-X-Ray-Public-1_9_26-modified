...
{	
	if (data.action.time_out !=0) {
		if (time_state_started + data.action.time_out < CRenderDevice::GetInstance()->dwTimeGlobal) return true;
	} 
...
{	
	if ( data.action.time_out != 0 )
	{
		if ( time_state_started + data.action.time_out < CRenderDevice::GetInstance()->dwTimeGlobal )
			return					true;
	} 
	...
	if (CRenderDevice::GetInstance()->dwTimeGlobal < time_state_started + 200 )
	{
