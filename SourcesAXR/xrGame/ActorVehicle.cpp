...
bool CActor::use_Vehicle(CHolderCustom* object)
{
	...
	if (m_holder)
	{
		if (!vehicle && m_holder->Use(CRenderDevice::GetInstance()->vCameraPosition, CRenderDevice::GetInstance()->vCameraDirection, center))
			detach_Vehicle();
		else
		{
			if (m_holder == vehicle)
				if (m_holder->Use(CRenderDevice::GetInstance()->vCameraPosition, CRenderDevice::GetInstance()->vCameraDirection, center))
					detach_Vehicle();
		}
		return true;
	}
	else
	{
		if (vehicle)
		{
			if (vehicle->Use(CRenderDevice::GetInstance()->vCameraPosition, CRenderDevice::GetInstance()->vCameraDirection, center))
			{
				if (pCamBobbing)
				{
