...
xrClientData::xrClientData	() :
	IClient(CRenderDevice::GetInstance()->GetTimerGlobal())
{
	ps = NULL;
	Clear		();
}
...
xrServer::xrServer() : IPureServer(CRenderDevice::GetInstance()->GetTimerGlobal(), g_dedicated_server)
{
...
void xrServer::Update	()
{
	...
	// spawn queue
	u32 svT								= CRenderDevice::GetInstance()->TimerAsync();
	...
	if( 0==(CRenderDevice::GetInstance()->dwFrame%100) )//once per 100 frames
	{
		UpdateBannedList();
	}
}
...
void xrServer::SendUpdatesToAll()
{
	...
	if ((CRenderDevice::GetInstance()->dwTimeGlobal - m_last_update_time) >= static_cast<u32>(1000 / psNET_ServerUpdate))
	{
		MakeUpdatePackets				();
	...
	case M_REMOTE_CONTROL_AUTH:
		{
			...
			}else
			{
				...
				if(res){
					...
					CL->m_admin_rights.m_dwLoginTime		= CRenderDevice::GetInstance()->dwTimeGlobal;
					if (CL->ps)
					{
...
if( dbg_net_Draw_Flags.test( dbg_destroy ) )
	...
	R_ASSERT					(P);
	entities.erase				(P->ID);
	m_tID_Generator.vfFreeID	(P->ID, CRenderDevice::GetInstance()->TimerAsync());
	...
	struct MaxPingClientDisconnector
	{
		...
		void operator()(IClient* client)
		{
			...
			if(	ps->ping > g_sv_dwMaxClientPing && 
				Client->m_ping_warn.m_dwLastMaxPingWarningTime+g_sv_time_for_ping_check < CRenderDevice::GetInstance()->dwTimeGlobal )
			{
				++Client->m_ping_warn.m_maxPingWarnings;
				Client->m_ping_warn.m_dwLastMaxPingWarningTime	= CRenderDevice::GetInstance()->dwTimeGlobal;
...
void xrServer::GetServerInfo( CServerInfo* si )
{
	...
	LPCSTR time = InventoryUtilities::GetTimeAsString(CRenderDevice::GetInstance()->dwTimeGlobal, InventoryUtilities::etpTimeToSecondsAndDay ).c_str();
