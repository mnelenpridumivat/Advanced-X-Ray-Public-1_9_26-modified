...
void CPlanner::update				()
{
	...
	if (m_use_log) {
		if (m_solution_changed) {
			...
			Msg						("%6d : Solution for object %s [%d vertices searched]", CRenderDevice::GetInstance()->dwTimeGlobal,object_name(),ai().graph_engine().solver_algorithm().data_storage().get_visited_node_count());
	...
	if (m_failed) {
		...
		Msg							("Time : %6d", CRenderDevice::GetInstance()->dwTimeGlobal);
