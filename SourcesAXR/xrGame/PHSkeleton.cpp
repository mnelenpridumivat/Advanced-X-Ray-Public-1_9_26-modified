...
void CPHSkeleton::Update(u32 dt)
{
	...
	if(b_removing&&
		CRenderDevice::GetInstance()->dwTimeGlobal>m_remove_time&&
		//(CRenderDevice::GetInstance()->dwTimeGlobal-m_unsplit_time)*phTimefactor>remove_time&&
...
{
	b_removing=true;
	m_remove_time= CRenderDevice::GetInstance()->dwTimeGlobal+iFloor(time/phTimefactor);
	SetNotNeedSave();
