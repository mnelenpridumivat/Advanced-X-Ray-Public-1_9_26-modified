	...
	case ATTACK:
		{
			if(actor)
			{
				...
				if(CRenderDevice::GetInstance()->dwTimeGlobal - fight_data_from->attack_time < min_attack_delta_time)
					break;

				fight_data_from->attack_time = CRenderDevice::GetInstance()->dwTimeGlobal;
