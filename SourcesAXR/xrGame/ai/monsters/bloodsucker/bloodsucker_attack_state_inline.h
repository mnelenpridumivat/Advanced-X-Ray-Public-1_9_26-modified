...
void   CStateMonsterBackstubEnemy<_Object>::initialize ()
{
	...
	m_encircle_end_tick = CRenderDevice::GetInstance()->dwTimeGlobal + detail::bloodsucker::encircle_time;
...
void   CStateMonsterBackstubEnemy<_Object>::execute ()
{
	...
	if ( object->conditions().GetHealth() < m_last_health-detail::bloodsucker::loose_health_diff &&
		 CRenderDevice::GetInstance()->dwTimeGlobal > m_next_change_behaviour_tick )
	{
		m_next_change_behaviour_tick = CRenderDevice::GetInstance()->dwTimeGlobal + detail::bloodsucker::change_behaviour_time;
		m_last_health = object->conditions().GetHealth();
	...
	if ( CRenderDevice::GetInstance()->dwTimeGlobal > m_encircle_end_tick ) 
	{
