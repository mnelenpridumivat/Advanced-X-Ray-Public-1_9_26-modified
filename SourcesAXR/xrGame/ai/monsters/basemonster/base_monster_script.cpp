...
void CBaseMonster::GenerateNewOffsetFromLeader ()
{
	...
	m_offset_from_leader_chosen_tick	= CRenderDevice::GetInstance()->dwTimeGlobal;
}
	...
	switch (l_tMovementAction.m_tGoalType) {
		...
		case CScriptMovementAction::eGoalTypeFollowLeader:
		{
			...
			else
			{
				...
				if (CRenderDevice::GetInstance()->dwTimeGlobal > m_offset_from_leader_chosen_tick + 5000 )
				{
...
void CBaseMonster::ProcessScripts()
{
	...
	CRenderDevice::GetInstance()->dwTimeGlobal							= CRenderDevice::GetInstance()->dwTimeGlobal;
