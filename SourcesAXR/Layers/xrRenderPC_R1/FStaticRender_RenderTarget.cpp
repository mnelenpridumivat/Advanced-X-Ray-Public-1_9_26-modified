...
{
	curWidth			= CRenderDevice::GetInstance()->dwWidth;
	curHeight			= CRenderDevice::GetInstance()->dwHeight;

	// Select mode to operate in
	float	amount		= ps_r__Supersample?float(ps_r__Supersample):1	;
	float	scale		= _sqrt	(amount);
	rtWidth				= clampr(iFloor(scale* CRenderDevice::GetInstance()->dwWidth  + .5f), 128, 2048);
	rtHeight			= clampr(iFloor(scale* CRenderDevice::GetInstance()->dwHeight + .5f), 128, 2048);
	...
	rt_ui_pda.create(RTname_pda_ui, CRenderDevice::GetInstance()->dwWidth, CRenderDevice::GetInstance()->dwHeight, HW.Caps.fTarget);

	if ((rtHeight!= CRenderDevice::GetInstance()->dwHeight) || (rtWidth!= CRenderDevice::GetInstance()->dwWidth))	{
...
//.	if (bDebug)	Msg			("%d,%d,%f",tw,th,param_noise_scale);
	...
	im_noise_time					-= CRenderDevice::GetInstance()->fTimeDelta;
	if (im_noise_time<0)			{
	...
	u32			_w					= CRenderDevice::GetInstance()->dwWidth;
	u32			_h					= CRenderDevice::GetInstance()->dwHeight;
	u32			cnt_w				= _w / tw;
	u32			cnt_h				= _h / th;
...
void CRenderTarget::calc_tc_duality_ss	(Fvector2& r0, Fvector2& r1, Fvector2& l0, Fvector2& l1)
{
	...
	if (rtHeight!= CRenderDevice::GetInstance()->dwHeight)	param_blur = 1.f;
...
{
	return Available() && 
		( ((BOOL)RImplementation.m_bMakeAsyncSS) || NeedPostProcess() || (ps_r__Supersample>1) || (frame_distort==(CRenderDevice::GetInstance()->dwFrame-1)));
}
...
void CRenderTarget::Begin		()
{
	...
	*/
	...
	CRenderDevice::GetInstance()->Clear				();
}
...
{
	frame_distort								= CRenderDevice::GetInstance()->dwFrame;
