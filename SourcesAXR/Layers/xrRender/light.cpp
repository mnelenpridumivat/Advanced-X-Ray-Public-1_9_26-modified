...
{
	if	(CRenderDevice::GetInstance()->dwFrame == m_xform_frame)	return;
	m_xform_frame	= CRenderDevice::GetInstance()->dwFrame;
...
float	light::get_LOD					()
{
	...
	float	distSQ			= CRenderDevice::GetInstance()->vCameraPosition.distance_to_sqr(spatial.sphere.P)+EPS;
