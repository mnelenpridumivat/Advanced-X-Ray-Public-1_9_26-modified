...
bool CStalkerAnimationManager::need_look_back				() const
{
	...
	if ((m_change_direction_time + need_look_back_time_delay) > CRenderDevice::GetInstance()->dwTimeGlobal)
		return					(false);
...
void CStalkerAnimationManager::legs_assign_direction		(float switch_factor, const EMovementDirection &direction)
{
	if (m_current_direction == direction) {
		m_direction_start		= CRenderDevice::GetInstance()->dwTimeGlobal;
		return;
	}
	...
	VERIFY						(m_direction_start <= CRenderDevice::GetInstance()->dwTimeGlobal);
	if ((CRenderDevice::GetInstance()->dwTimeGlobal - m_direction_start) <= static_cast<u32>(iFloor(switch_factor * direction_switch_interval)))
		return;
...
MotionID CStalkerAnimationManager::legs_move_animation		()
{
	...
	if (m_previous_speed_direction != speed_direction) {
		if (m_change_direction_time < CRenderDevice::GetInstance()->dwTimeGlobal)
			m_change_direction_time	= CRenderDevice::GetInstance()->dwTimeGlobal;
