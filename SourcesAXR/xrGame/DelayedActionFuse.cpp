...
void CDelayedActionFuse::SetTimer(float current_condition)
{
	...
	m_fTime+= CRenderDevice::GetInstance()->fTimeGlobal;//+current_condition/m_fSpeedChangeCondition;
...
float CDelayedActionFuse::Time()
{
	...
	else			return m_fTime- CRenderDevice::GetInstance()->fTimeGlobal;
}
...
bool CDelayedActionFuse::Update(float current_condition)
{
	...
	bool ret=false;
	float l_time_to_explosion=m_fTime- CRenderDevice::GetInstance()->fTimeGlobal;
