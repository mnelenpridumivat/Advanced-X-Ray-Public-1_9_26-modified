...
void SHeliMovementState::goByRoundPath(Fvector center_, float radius_, bool clockwise_)
{
	...
	for(;it!=it_e;++it,++pt_idx){
		...
		CPatrolPoint pt = CPatrolPoint((CLevelGraph*)nullptr,(CGameLevelCrossTable*)nullptr,(CGameGraph*)nullptr,pp,it->point,static_cast<u32>(-1),0,pt_name);
...
void SHeliMovementState::SetPointFlags(u32 idx, u32 new_flags)
{
	...
	CPatrolPoint* pt_new	= xr_new<CPatrolPoint>(	
		static_cast<CLevelGraph*>(nullptr),
		static_cast<CGameLevelCrossTable*>(nullptr),
		static_cast<CGameGraph*>(nullptr),
		currPatrolPath,
		pt_curr->position(),
		static_cast<u32>(-1),
		new_flags,
