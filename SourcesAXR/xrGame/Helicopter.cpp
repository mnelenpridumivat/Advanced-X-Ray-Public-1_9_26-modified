...
BOOL CHelicopter::net_Spawn(CSE_Abstract*	DC)
{
	...
	if( pUserData->section_exist(s) ){
		...
		for (int i=0 ;i<lc; ++i){
			...
			m_hitBones.insert( std::make_pair(boneID, static_cast<float>(atof(value))) );
		}
	}
...
#ifdef DEBUG
	CRenderDevice::GetInstance()->seqRender.Add(this,REG_PRIORITY_LOW-1);
#endif
...
void CHelicopter::UpdateCL()
{
	...
	m_stepRemains+= CRenderDevice::GetInstance()->fTimeDelta;
