...
void obstacles_query::set_intersection	(const obstacles_query &query)
{
	...
	OBSTACLES::value_type		*temp = static_cast<OBSTACLES::value_type*>(_alloca(buffer_size));
...
void obstacles_query::merge				(const AREA &object_area)
{
	...
	u32							*temp = static_cast<u32*>(_alloca(buffer_size));
...
void obstacles_query::compute_area		()
{
	...
	m_area.erase(m_area.begin(), m_area.end());
