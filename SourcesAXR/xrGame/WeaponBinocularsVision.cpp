...
void SBinocVisibleObj::Update()
{
	...
	Fmatrix				xform;
	xform.mul			(CRenderDevice::GetInstance()->mFullTransform,m_object->XFORM());
	...
	if (m_flags.is(flTargetLocked)){
		...
		cur_rect.lt.x	+= (mn.x-cur_rect.lt.x)*m_upd_speed* CRenderDevice::GetInstance()->fTimeDelta;
		cur_rect.lt.y	+= (mn.y-cur_rect.lt.y)*m_upd_speed* CRenderDevice::GetInstance()->fTimeDelta;
		cur_rect.rb.x	+= (mx.x-cur_rect.rb.x)*m_upd_speed* CRenderDevice::GetInstance()->fTimeDelta;
		cur_rect.rb.y	+= (mx.y-cur_rect.rb.y)*m_upd_speed* CRenderDevice::GetInstance()->fTimeDelta;
