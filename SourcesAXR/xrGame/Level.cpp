...
CLevel::CLevel():IPureClient	(CRenderDevice::GetInstance()->GetTimerGlobal())
...
//					GO->H_Parent()->ID(), GO->H_Parent()->cName().c_str(), CRenderDevice::GetInstance()->dwFrame );
//			}
		}
		GO->OnEvent		(P,type);
	}
...
void CLevel::ProcessGameEvents		()
{
	// Game events
	{
		...
		if (!game_events->queue.empty())	
			Msg("- d[%d],ts[%d] -- E[svT=%d],[evT=%d]",CRenderDevice::GetInstance()->dwTimeGlobal,timeServer(),svT,game_events->queue.begin()->timestamp);
		*/
...
void CLevel::OnFrame	()
{
	...
	CRenderDevice::GetInstance()->Statistic->TEST0.Begin		();
	BulletManager().CommitEvents		();
	CRenderDevice::GetInstance()->Statistic->TEST0.End			();
	...
	if (net_isDisconnected())	
	{
		...
		CRenderDevice::GetInstance()->Statistic->netClient1.Begin();

		ClientReceive					();

		CRenderDevice::GetInstance()->Statistic->netClient1.End	();
	}
	...
	if(!g_dedicated_server )
	{
		if (g_mt_config.test(mtMap)) 
			CRenderDevice::GetInstance()->seqParallel.push_back	(fastdelegate::FastDelegate0<>(m_map_manager,&CMapManager::Update));
		else								
			MapManager().Update		();

		if( IsGameTypeSingle() && CRenderDevice::GetInstance()->dwPrecacheFrame==0 )
		{
			//if (g_mt_config.test(mtMap)) 
			//	CRenderDevice::GetInstance()->seqParallel.push_back	(fastdelegate::FastDelegate0<>(m_game_task_manager,&CGameTaskManager::UpdateTasks));
			//else								
	...
	//CRenderDevice::GetInstance()->Statistic->cripting.Begin	();
	if (!g_dedicated_server)
		ai().script_engine().script_process	(ScriptEngine::eScriptProcessorLevel)->update();
	//CRenderDevice::GetInstance()->Statistic->Scripting.End	();
...
//	autosave_manager().update			();
	...
	if(!g_dedicated_server)
	{
		if (g_mt_config.test(mtLevelSounds)) 
			CRenderDevice::GetInstance()->seqParallel.push_back	(fastdelegate::FastDelegate0<>(m_level_sound_manager,&CLevelSoundManager::Update));
		else								
	...
	if (!g_dedicated_server)
	{
		if (g_mt_config.test(mtLUA_GC))	CRenderDevice::GetInstance()->seqParallel.push_back	(fastdelegate::FastDelegate0<>(this,&CLevel::script_gc));
		else							script_gc	()	;
	}
	//-----------------------------------------------------
	if (pStatGraphR)
	...
	if (bDebug)	{
		for (u32 I=0; I < Level().Objects.o_count(); I++) {
			...
			if (dbg_net_Draw_Flags.test(dbg_draw_skeleton)) //draw skeleton
			{
				...
				{
					if (pGO->Position().distance_to_sqr(CRenderDevice::GetInstance()->vCameraPosition) < 400.0f)
					{
