...
void CAI_Stalker::on_best_cover_changed				(const CCoverPoint *new_cover, const CCoverPoint *old_cover)
{
	...
	if (new_cover) {
		if (!new_cover->m_is_smart_cover)
			Msg							("[%6d][%s], now it is cover", CRenderDevice::GetInstance()->dwTimeGlobal, cName().c_str());
		else
			Msg							(
...
{
#ifdef _DEBUG
//	Msg									("* [%6d][%s] search for new cover performed",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
#ifdef _DEBUG
...
void CAI_Stalker::update_best_cover_actuality		(const Fvector &position_to_cover_from)
{
	...
	if (m_best_cover->position().distance_to_sqr(position_to_cover_from) < _sqr(MIN_SUITABLE_ENEMY_DISTANCE)) {
		m_best_cover_actual				= false;
#if 0//def _DEBUG
		Msg								("* [%6d][%s] enemy too close",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
		return;
	}
	...
	if (cover_value >= m_best_cover_value + 1.f) {
		m_best_cover_actual				= false;
#if 0//def _DEBUG
		Msg								("* [%6d][%s] cover became too bad",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
		return;
	}
...
//		m_best_cover_actual				= false;
//		Msg								("* [%6d][%s] cover became too bad2",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
//		return;
//	}
...
#ifdef _DEBUG
//	Msg									("* [%6d][%s] advance search performed",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
#ifdef _DEBUG
...
void CAI_Stalker::on_restrictions_change			()
{
	...
	m_best_cover_actual					= false;
#ifdef _DEBUG
	Msg									("* [%6d][%s] on_restrictions_change",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
}
...
void CAI_Stalker::on_enemy_change					(const CEntityAlive *enemy)
{
	...
	m_best_cover_actual					= false;
#ifdef _DEBUG
//	Msg									("* [%6d][%s] on_enemy_change",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
}
...
#ifdef _DEBUG
//		Msg								("* [%6d][%s] on_danger_add",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
		m_best_cover_actual				= false;
	}
}
...
#ifdef _DEBUG
//			Msg							("* [%6d][%s] on_danger_remove",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
			m_best_cover_actual			= false;
		}
...
{
#ifdef _DEBUG
//	Msg									("* [%6d][%s] cover is blocked",CRenderDevice::GetInstance()->dwTimeGlobal,*cName());
#endif
	m_best_cover_actual					= false;
}
