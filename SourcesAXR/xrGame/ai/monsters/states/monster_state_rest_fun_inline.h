...
void CStateMonsterRestFunAbstract::execute()
{
	...
	if ((dist < object->db().m_fDistToCorpse + 0.5f) && (time_last_hit + MIN_DELAY < CRenderDevice::GetInstance()->dwTimeGlobal)) {
		...
		if  (target && target->m_pPhysicsShell) {
			...
			time_last_hit	= CRenderDevice::GetInstance()->dwTimeGlobal;
		}
	}
}
...
bool CStateMonsterRestFunAbstract::check_completion()
{
	...
	if (time_state_started + TIME_IN_STATE < CRenderDevice::GetInstance()->dwTimeGlobal) return true;
	return false;
}
