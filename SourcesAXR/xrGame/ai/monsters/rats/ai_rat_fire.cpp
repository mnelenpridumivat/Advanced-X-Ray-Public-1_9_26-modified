...
void CAI_Rat::HitSignal(float amount, Fvector& vLocalDir, CObject* who, s16 /**element/**/)
{
	...
	m_hit_time = CRenderDevice::GetInstance()->dwTimeGlobal;
...
float CAI_Rat::evaluate		(const CItemManager *manager, const CGameObject *object) const
{
	...
	if (!entity_alive->g_Alive()) {
		if ((CRenderDevice::GetInstance()->dwTimeGlobal - entity_alive->GetLevelDeathTime() < m_dwEatCorpseInterval) && (entity_alive->m_fFood > 0) && (m_bEatMemberCorpses || (entity_alive->g_Team() != g_Team())) && (m_bCannibalism || (entity_alive->CLS_ID != CLS_ID)))
...
{
	u32					dwCurTime = CRenderDevice::GetInstance()->dwTimeGlobal;
