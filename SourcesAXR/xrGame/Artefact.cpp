...
{
	float uncharge_coef = (m_fDegradationSpeed / 16) * CRenderDevice::GetInstance()->fTimeDelta;
	...
	{
		if (!fis_zero(m_HitTypeProtection[static_cast<ALife::EInfluenceType>(i)]) && !fis_zero(m_ConstHitTypeProtection[static_cast<ALife::EInfluenceType>(i)]))
			m_HitTypeProtection[static_cast<ALife::EInfluenceType>(i)] = (m_ConstHitTypeProtection[static_cast<ALife::EInfluenceType>(i)] / 100) * percent;
	}
...
void CArtefact::shedule_Update		(u32 dt) 
{
	...
	if (H_Parent())			o_switch_2_slow	();
		...
		BOOL	rendering		= (CRenderDevice::GetInstance()->dwFrame==o_render_frame);
		float	cam_distance	= CRenderDevice::GetInstance()->vCameraPosition.distance_to(center)-Radius();
...
{
	m_switchVisTime			= CRenderDevice::GetInstance()->dwTimeGlobal;
...
void SArtefactDetectorsSupport::UpdateOnFrame()
{
	...
	if(m_parent->getVisible() && m_parent->GetAfRank()!=0 && m_switchVisTime+5000 < CRenderDevice::GetInstance()->dwTimeGlobal)
		SetVisible(false);

	u32 dwDt = 2*3600*1000/10; //2 hour of game time
	if(!m_parent->getVisible() && m_switchVisTime+dwDt < CRenderDevice::GetInstance()->dwTimeGlobal)
	{
		m_switchVisTime		= CRenderDevice::GetInstance()->dwTimeGlobal;
		if(m_parent->Position().distance_to(CRenderDevice::GetInstance()->vCameraPosition)>40.0f)
			Blink			();
	}
}
