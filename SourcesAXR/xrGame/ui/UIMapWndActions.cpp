...
public:
					CSomeMapAction		(LPCSTR action_name):inherited(static_cast<CUIMapWnd*>(NULL),action_name){}
	void	initialize			() override {inherited::initialize	();};
	void	execute				() override {inherited::execute		();};
	void	finalize			() override {inherited::finalize	();};
};
...
public:
					CSomeMapEvaluator	(LPCSTR evaluator_name):inherited(static_cast<CUIMapWnd*>(NULL),evaluator_name){}
	~CSomeMapEvaluator	() override {};
};
...
public:
	...
	bool	evaluate			() override {return ret_value;};
...
void CMapActionZoomControl::init_internal()
{
	...
	m_endMovingTime				= CRenderDevice::GetInstance()->fTimeGlobal;
...
void CMapActionZoomControl::execute		()
{
	...
	float gt				= CRenderDevice::GetInstance()->fTimeGlobal;
	float time_to			= m_endMovingTime-gt;
	float dt				= _min(CRenderDevice::GetInstance()->fTimeDelta,time_to);

	if(m_endMovingTime > CRenderDevice::GetInstance()->fTimeGlobal)
	{
...
void CMapActionMinimize::initialize()
{
	...
	inherited::initialize		();
	m_endMovingTime				= CRenderDevice::GetInstance()->fTimeGlobal+map_zoom_time;
}
