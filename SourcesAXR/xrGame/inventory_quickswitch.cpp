...
{
	if (m_next_item_iteration_time + exception_items_clear_time <= CRenderDevice::GetInstance()->dwTimeGlobal)
	{
	...
	if (best_fit)
	{
		...
		m_next_item_iteration_time = CRenderDevice::GetInstance()->dwTimeGlobal;
		return best_fit;
	}
