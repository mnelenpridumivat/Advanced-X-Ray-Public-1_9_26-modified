...
bool CAI_Stalker::useful		(const CItemManager *manager, const CGameObject *object) const
{
	...
	if (explosive && smart_cast<const CInventoryItem*>(object))
		agent_manager().location().add	(xr_new<CDangerObjectLocation>(object, CRenderDevice::GetInstance()->dwTimeGlobal,DANGER_INFINITE_INTERVAL,DANGER_EXPLOSIVE_DISTANCE));
	...
	if (explosive && (explosive->CurrentParentID() != 0xffff)) {
		...
		if (entity_alive)
			memory().danger().add(CDangerObject(entity_alive,object->Position(), CRenderDevice::GetInstance()->dwTimeGlobal,CDangerObject::eDangerTypeGrenade,CDangerObject::eDangerPerceiveTypeVisual,object));
	}
...
void CAI_Stalker::react_on_grenades		()
{
	...
	if (CRenderDevice::GetInstance()->dwTimeGlobal < reaction.m_time + GRENADE_INTERVAL)
		return;

//	u32							interval = AFTER_GRENADE_DESTROYED_INTERVAL;
	const CMissile				*missile = smart_cast<const CMissile*>(reaction.m_grenade);
//	if (missile && (missile->destroy_time() > CRenderDevice::GetInstance()->dwTimeGlobal))
//		interval				= missile->destroy_time() - CRenderDevice::GetInstance()->dwTimeGlobal + AFTER_GRENADE_DESTROYED_INTERVAL;
//	m_object->agent_manager().add_danger_location(reaction.m_game_object->Position(),CRenderDevice::GetInstance()->dwTimeGlobal,interval,GRENADE_RADIUS);

	if (missile && agent_manager().member().group_behaviour()) {
//		Msg						("%6d : Stalker %s : grenade reaction",CRenderDevice::GetInstance()->dwTimeGlobal,*m_object->cName());
		...
		if (initiator) {
			if (is_relation_enemy(initiator))
				...
				if (missile->Position().distance_to(Position()) < FRIENDLY_GRENADE_ALARM_DIST) {
					u32 const time	= missile->destroy_time() >= CRenderDevice::GetInstance()->dwTimeGlobal ? static_cast<u32>(missile->destroy_time() - CRenderDevice::GetInstance()->dwTimeGlobal) : 0;
...
void CAI_Stalker::react_on_member_death	()
{
	...
	if (CRenderDevice::GetInstance()->dwTimeGlobal < reaction.m_time + TOLLS_INTERVAL)
		return;
