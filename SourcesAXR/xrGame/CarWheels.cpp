...
void CCar::SWheelDrive::Init()
{
	...
	CBoneData& bone_data= reinterpret_cast<IKinematics*>(pwheel->car->Visual())->LL_GetData(static_cast<u16>(pwheel->bone_id));
...
{
	IKinematics* pKinematics= reinterpret_cast<IKinematics*>(pwheel->car->Visual());
	pwheel->Init();
