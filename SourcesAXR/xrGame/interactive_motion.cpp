...
void interactive_motion::setup( LPCSTR m, CPhysicsShell *s, float angle )
{
	VERIFY( m );
	VERIFY( s );
	IKinematicsAnimated* K = reinterpret_cast<IKinematicsAnimated*>( s->PKinematics( ) );
	VERIFY( K );
...
void interactive_motion::play( )
{
	VERIFY( shell );
	VERIFY( motion.valid() );
	reinterpret_cast<IKinematicsAnimated*>( shell->PKinematics( ) )->PlayCycle( motion, TRUE, anim_callback, this );
	state_start( );
}
