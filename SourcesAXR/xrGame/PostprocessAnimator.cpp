...
BOOL CPostprocessAnimator::Process(SPPInfo &PPInfo)
{
	...
	if(m_start_time<0.0f)m_start_time= CRenderDevice::GetInstance()->fTimeGlobal;
	if(m_bCyclic &&((CRenderDevice::GetInstance()->fTimeGlobal-m_start_time)>f_length)) m_start_time+=f_length;

	Update					(CRenderDevice::GetInstance()->fTimeGlobal-m_start_time);

	VERIFY				(_valid(m_factor));
	VERIFY				(_valid(m_factor_speed));
	VERIFY				(_valid(m_dest_factor));
	if(m_bStop)
		m_factor			-= CRenderDevice::GetInstance()->fTimeDelta*m_factor_speed;
	else
		m_factor			+= m_factor_speed* CRenderDevice::GetInstance()->fTimeDelta*(m_dest_factor-m_factor);
