...
BOOL CAnimatorCamEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	m_objectAnimator->Update	(CRenderDevice::GetInstance()->fTimeDelta);
	...
	if(!m_bAbsolutePositioning){
		...
		Mdef.c						= info.p;
//		Msg("fr[%d] %2.3f,%2.3f,%2.3f", CRenderDevice::GetInstance()->dwFrame,m.c.x,m.c.y,m.c.z);
		Fmatrix mr;
		mr.mul						(Mdef,m);
		info.d						= mr.k;
		info.n						= mr.j;
...
{
	float f				= (m_end_time- CRenderDevice::GetInstance()->fTimeGlobal)/m_life_time;
...
void SndShockEffector::Start(CActor* A, float snd_length, float power)
{
	...
	m_end_time			= CRenderDevice::GetInstance()->fTimeGlobal + m_life_time;
...
{
	m_cur_length		+= CRenderDevice::GetInstance()->dwTimeDelta;
...
BOOL CControllerPsyHitCamEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	if (angle_lerp(m_dangle_current.x, m_dangle_target.x, ANGLE_SPEED, CRenderDevice::GetInstance()->fTimeDelta)) {
		m_dangle_target.x = angle_normalize(Random.randFs(DELTA_ANGLE_X));
	}

	if (angle_lerp(m_dangle_current.y, m_dangle_target.y, ANGLE_SPEED, CRenderDevice::GetInstance()->fTimeDelta)) {
		m_dangle_target.y = angle_normalize(Random.randFs(DELTA_ANGLE_Y));
	}

	if (angle_lerp(m_dangle_current.z, m_dangle_target.z, ANGLE_SPEED, CRenderDevice::GetInstance()->fTimeDelta)) {
	...
	m_time_current	+= CRenderDevice::GetInstance()->fTimeDelta;
