...
void CSoundPlayer::play				(u32 internal_type, u32 max_start_time, u32 min_start_time, u32 max_stop_time, u32 min_stop_time, u32 id)
{
	...
	sound_single.m_bone_id		= reinterpret_cast<IKinematics*>(m_object->Visual())->LL_BoneID(sound.m_bone_name);
...
{
	Fmatrix						l_tMatrix;
	l_tMatrix.mul_43			(m_object->XFORM(), reinterpret_cast<IKinematics*>(m_object->Visual())->LL_GetBoneInstance(sound.m_bone_id).mTransform);
	return						(l_tMatrix.c);
}
