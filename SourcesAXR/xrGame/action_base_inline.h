...
#endif
	m_start_level_time	= CRenderDevice::GetInstance()->dwTimeGlobal;
	m_first_time		= true;
}
...
{
	return				(m_start_level_time + m_inertia_time <= CRenderDevice::GetInstance()->dwTimeGlobal);
}
...
IC	void CBaseAction::debug_log			(const EActionStates state_state) const
{
	switch (state_state) {
		case eActionStateConstructed : {
			Msg			("[%6d] action %s is constructed", CRenderDevice::GetInstance()->dwTimeGlobal,m_action_name);
			break;
		}
		case eActionStateSetup : {
			Msg			("[%6d] action %s is setup", CRenderDevice::GetInstance()->dwTimeGlobal,m_action_name);
			break;
		}
		case eActionStateInitialized : {
			Msg			("[%6d] action %s is initialized", CRenderDevice::GetInstance()->dwTimeGlobal,m_action_name);
			break;
		}
		case eActionStateExecuted : {
			Msg			("[%6d] action %s is executed", CRenderDevice::GetInstance()->dwTimeGlobal,m_action_name);
			break;
		}
		case eActionStateFinalized : {
			Msg			("[%6d] action %s is finalized", CRenderDevice::GetInstance()->dwTimeGlobal,m_action_name);
			break;
		}
		default : NODEFAULT;
	}
