...
_value_type CStalkerPropertyEvaluatorEnemies::evaluate	()
{
	...
	if (CRenderDevice::GetInstance()->dwTimeGlobal < m_object->memory().enemy().last_enemy_time() + m_time_to_wait)
		return			(true);
...
_value_type CStalkerPropertyEvaluatorShouldThrowGrenade::evaluate	()
{
	...
	if (object().last_throw_time() + object().throw_time_interval() >= CRenderDevice::GetInstance()->dwTimeGlobal)
		return					(false);
