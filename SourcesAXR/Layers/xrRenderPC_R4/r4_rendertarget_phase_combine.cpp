...
void	CRenderTarget::phase_combine	()
{
	...
	//*** exposure-pipeline
	u32			gpu_id	= CRenderDevice::GetInstance()->dwFrame%HW.Caps.iGPUNum;
	if (RImplementation.currentViewPort == SECONDARY_WEAPON_SCOPE) //--#SM+#-- +SecondVP+
	{
		// clang-format off
		gpu_id = (CRenderDevice::GetInstance()->dwFrame - 1) % HW.Caps.iGPUNum;	//  "" tonemapping (HDR)    . 
															//   -       tonemapping (HDR)    
															//    ,  HDR       " "
															//       ,     
															//        ,        \
	...
	Fvector2	m_blur_scale;
	{
		...
		Fmatrix	m_invview;	m_invview.invert	(CRenderDevice::GetInstance()->mView);
		m_previous.mul		(m_saved_viewproj,m_invview);
		m_current.set		(CRenderDevice::GetInstance()->mProject)		;
		m_saved_viewproj.set(CRenderDevice::GetInstance()->mFullTransform)	;
...
//.		Fvector4	envclr			= { envdesc.sky_color.x*2+EPS,	envdesc.sky_color.y*2+EPS,	envdesc.sky_color.z*2+EPS,	envdesc.weight					};
		...
		float		fSSAONoise = 2.0f;
					...
					fSSAONoise /= tan(deg2rad(CRenderDevice::GetInstance()->fFOV/2.0f));

		float		fSSAOKernelSize = 150.0f;
					fSSAOKernelSize *= tan(deg2rad(67.5f/2.0f));
					fSSAOKernelSize /= tan(deg2rad(CRenderDevice::GetInstance()->fFOV/2.0f));
...
/*
   ...
   */
   ...
   if( RImplementation.o.dx10_msaa )
   {
	   ...
	   else					   u_setrt		(CRenderDevice::GetInstance()->dwWidth, CRenderDevice::GetInstance()->dwHeight,HW.pBaseRT,NULL,NULL,HW.pBaseZB);
   }
   else
   {
