...
void SStaticSound::Update(u32 game_time, u32 global_time)
{
	...
	if ((0==m_ActiveTime.x)&&(0==m_ActiveTime.y)||((static_cast<int>(game_time)>=m_ActiveTime.x)&&(static_cast<int>(game_time)<m_ActiveTime.y))){
		if (0==m_Source._feedback()){
			...
			if (CRenderDevice::GetInstance()->dwTimeGlobal>=m_StopTime)
...
void CLevelSoundManager::Update()
{
	...
	if (CRenderDevice::GetInstance()->dwPrecacheFrame!=0)		return;
	// static sounds
	u32 game_time						= Level().GetGameDayTimeMS();
	u32 engine_time						= CRenderDevice::GetInstance()->dwTimeGlobal;
