...
void CObjectHandler::reinit			(CAI_Stalker *object)
{
	...
	m_strap_object_id			= static_cast<ALife::_OBJECT_ID>(-1);
	m_strap_bone0				= -1;
	m_strap_bone1				= -1;
...
//		Msg						( "[%6d][%s] actualizing: weapon_strapped = false", CRenderDevice::GetInstance()->dwTimeGlobal, planner().object().cName().c_str() );
		weapon->strapped_mode	(false);
		return;
	}

	THROW3						(weapon->can_be_strapped(),"Cannot strap weapon",*weapon->cName());
//	Msg							( "[%6d][%s] actualizing: weapon_strapped = true", CRenderDevice::GetInstance()->dwTimeGlobal, planner().object().cName().c_str() );
	weapon->strapped_mode		(true);
}
...
//		Msg						( "[%6d][%s] weapon_strapped = false0", CRenderDevice::GetInstance()->dwTimeGlobal, planner().object().cName().c_str() );
		return					(false);
	}
...
//		Msg						( "[%6d][%s] weapon_strapped = %s1", CRenderDevice::GetInstance()->dwTimeGlobal, planner().object().cName().c_str(), almost_strapped && !planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped2Idle) ? "true" : "false" );
...
bool CObjectHandler::weapon_unstrapped	() const
{
	CWeapon						*weapon = smart_cast<CWeapon*>(inventory().ActiveItem());
	if (!weapon) {
//		Msg						( "[%6d][%s] no active item!!(%d)(%s)", CRenderDevice::GetInstance()->dwTimeGlobal, planner().object().cName().c_str(), inventory().GetActiveSlot(), inventory().ItemFromSlot(3) ? inventory().ItemFromSlot(3)->object().cName().c_str() : "<no_item>" );
		return					(true);
	}
...
bool CObjectHandler::weapon_unstrapped	(CWeapon *weapon) const
{
	...
	if (!weapon->can_be_strapped()) {
//		Msg						( "[%6d][%s] weapon_unstrapped = true0", CRenderDevice::GetInstance()->dwTimeGlobal, planner().object().cName().c_str() );
		return					(true);
	}
...
//		Msg						( "[%6d][%s] weapon_unstrapped = %s1", CRenderDevice::GetInstance()->dwTimeGlobal, planner().object().cName().c_str(), almost_unstrapped && !planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped2Idle) ? "true" : "false" );
