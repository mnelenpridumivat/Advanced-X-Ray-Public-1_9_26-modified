...
bool CAI_Stalker::bfAssignMovement(CScriptEntityAction *tpEntityAction)
{
	...
	movement().update				(CRenderDevice::GetInstance()->dwTimeDelta);
	sight().update					();
...
#ifdef _DEBUG
//		Msg				("%6d Assigning animation : %s",CRenderDevice::GetInstance()->dwTimeGlobal,*tpEntityAction->m_tAnimationAction.m_caAnimationToPlay);
#endif
