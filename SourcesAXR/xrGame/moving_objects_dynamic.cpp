...
void moving_objects::fill_nearest_moving		(moving_object *object)
{
...
#if 0
	Msg							("%6d nearest moving[%d] object[%s]", CRenderDevice::GetInstance()->dwFrame, m_nearest_moving.size(),object->object().cName().c_str());
	{
	...
	struct already_computed {
		...
		{
			return				(object->action_frame() == CRenderDevice::GetInstance()->dwFrame);
		}
	};
...
void moving_objects::query_action_dynamic		(moving_object *object)
{
	...
	if (object->action_frame() == CRenderDevice::GetInstance()->dwFrame)
		return;
...
#if 0//def DEBUG
	Msg							("%6d end of iteration", CRenderDevice::GetInstance()->dwFrame);
#endif // DEBUG
...
#if 0//def DEBUG
	{
		Msg							("Frame[%d], collisions[%d]", CRenderDevice::GetInstance()->dwFrame, m_visited_emitters.size());
		NEAREST_MOVING::iterator	I = m_visited_emitters.begin();
		NEAREST_MOVING::iterator	E = m_visited_emitters.end();
		for ( ; I != E; ++I)
